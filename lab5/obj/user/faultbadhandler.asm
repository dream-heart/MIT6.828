
obj/user/faultbadhandler.debug：     文件格式 elf32-i386


Disassembly of section .text:

00800020 <_start>:
// starts us running when we are initially loaded into a new environment.
.text
.globl _start
_start:
	// See if we were started with arguments on the stack
	cmpl $USTACKTOP, %esp
  800020:	81 fc 00 e0 bf ee    	cmp    $0xeebfe000,%esp
	jne args_exist
  800026:	75 04                	jne    80002c <args_exist>

	// If not, push dummy argc/argv arguments.
	// This happens when we are loaded by the kernel,
	// because the kernel does not know about passing arguments.
	pushl $0
  800028:	6a 00                	push   $0x0
	pushl $0
  80002a:	6a 00                	push   $0x0

0080002c <args_exist>:

args_exist:
	call libmain
  80002c:	e8 44 00 00 00       	call   800075 <libmain>
1:	jmp 1b
  800031:	eb fe                	jmp    800031 <args_exist+0x5>

00800033 <umain>:

#include <inc/lib.h>

void
umain(int argc, char **argv)
{
  800033:	55                   	push   %ebp
  800034:	89 e5                	mov    %esp,%ebp
  800036:	83 ec 18             	sub    $0x18,%esp
	sys_page_alloc(0, (void*) (UXSTACKTOP - PGSIZE), PTE_P|PTE_U|PTE_W);
  800039:	c7 44 24 08 07 00 00 	movl   $0x7,0x8(%esp)
  800040:	00 
  800041:	c7 44 24 04 00 f0 bf 	movl   $0xeebff000,0x4(%esp)
  800048:	ee 
  800049:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  800050:	e8 49 01 00 00       	call   80019e <sys_page_alloc>
	sys_env_set_pgfault_upcall(0, (void*) 0xDeadBeef);
  800055:	c7 44 24 04 ef be ad 	movl   $0xdeadbeef,0x4(%esp)
  80005c:	de 
  80005d:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  800064:	e8 d5 02 00 00       	call   80033e <sys_env_set_pgfault_upcall>
	*(int*)0 = 0;
  800069:	c7 05 00 00 00 00 00 	movl   $0x0,0x0
  800070:	00 00 00 
}
  800073:	c9                   	leave  
  800074:	c3                   	ret    

00800075 <libmain>:
const volatile struct Env *thisenv;
const char *binaryname = "<unknown>";

void
libmain(int argc, char **argv)
{
  800075:	55                   	push   %ebp
  800076:	89 e5                	mov    %esp,%ebp
  800078:	56                   	push   %esi
  800079:	53                   	push   %ebx
  80007a:	83 ec 10             	sub    $0x10,%esp
  80007d:	8b 5d 08             	mov    0x8(%ebp),%ebx
  800080:	8b 75 0c             	mov    0xc(%ebp),%esi
	// set thisenv to point at our Env structure in envs[].
	// LAB : Your code here.
	envid_t envid = sys_getenvid();
  800083:	e8 d8 00 00 00       	call   800160 <sys_getenvid>
	thisenv = &envs[ENVX(envid)];
  800088:	25 ff 03 00 00       	and    $0x3ff,%eax
  80008d:	6b c0 7c             	imul   $0x7c,%eax,%eax
  800090:	05 00 00 c0 ee       	add    $0xeec00000,%eax
  800095:	a3 04 20 80 00       	mov    %eax,0x802004

	// save the name of the program so that panic() can use it
	if (argc > 0)
  80009a:	85 db                	test   %ebx,%ebx
  80009c:	7e 07                	jle    8000a5 <libmain+0x30>
		binaryname = argv[0];
  80009e:	8b 06                	mov    (%esi),%eax
  8000a0:	a3 00 20 80 00       	mov    %eax,0x802000

	// call user main routine
	umain(argc, argv);
  8000a5:	89 74 24 04          	mov    %esi,0x4(%esp)
  8000a9:	89 1c 24             	mov    %ebx,(%esp)
  8000ac:	e8 82 ff ff ff       	call   800033 <umain>

	// exit gracefully
	exit();
  8000b1:	e8 07 00 00 00       	call   8000bd <exit>
}
  8000b6:	83 c4 10             	add    $0x10,%esp
  8000b9:	5b                   	pop    %ebx
  8000ba:	5e                   	pop    %esi
  8000bb:	5d                   	pop    %ebp
  8000bc:	c3                   	ret    

008000bd <exit>:

#include <inc/lib.h>

void
exit(void)
{
  8000bd:	55                   	push   %ebp
  8000be:	89 e5                	mov    %esp,%ebp
  8000c0:	83 ec 18             	sub    $0x18,%esp
	//close_all();
	sys_env_destroy(0);
  8000c3:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
  8000ca:	e8 3f 00 00 00       	call   80010e <sys_env_destroy>
}
  8000cf:	c9                   	leave  
  8000d0:	c3                   	ret    

008000d1 <sys_cputs>:
	return ret;
}

void
sys_cputs(const char *s, size_t len)
{
  8000d1:	55                   	push   %ebp
  8000d2:	89 e5                	mov    %esp,%ebp
  8000d4:	57                   	push   %edi
  8000d5:	56                   	push   %esi
  8000d6:	53                   	push   %ebx
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8000d7:	b8 00 00 00 00       	mov    $0x0,%eax
  8000dc:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8000df:	8b 55 08             	mov    0x8(%ebp),%edx
  8000e2:	89 c3                	mov    %eax,%ebx
  8000e4:	89 c7                	mov    %eax,%edi
  8000e6:	89 c6                	mov    %eax,%esi
  8000e8:	cd 30                	int    $0x30

void
sys_cputs(const char *s, size_t len)
{
	syscall(SYS_cputs, 0, (uint32_t)s, len, 0, 0, 0);
}
  8000ea:	5b                   	pop    %ebx
  8000eb:	5e                   	pop    %esi
  8000ec:	5f                   	pop    %edi
  8000ed:	5d                   	pop    %ebp
  8000ee:	c3                   	ret    

008000ef <sys_cgetc>:

int
sys_cgetc(void)
{
  8000ef:	55                   	push   %ebp
  8000f0:	89 e5                	mov    %esp,%ebp
  8000f2:	57                   	push   %edi
  8000f3:	56                   	push   %esi
  8000f4:	53                   	push   %ebx
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8000f5:	ba 00 00 00 00       	mov    $0x0,%edx
  8000fa:	b8 01 00 00 00       	mov    $0x1,%eax
  8000ff:	89 d1                	mov    %edx,%ecx
  800101:	89 d3                	mov    %edx,%ebx
  800103:	89 d7                	mov    %edx,%edi
  800105:	89 d6                	mov    %edx,%esi
  800107:	cd 30                	int    $0x30

int
sys_cgetc(void)
{
	return syscall(SYS_cgetc, 0, 0, 0, 0, 0, 0);
}
  800109:	5b                   	pop    %ebx
  80010a:	5e                   	pop    %esi
  80010b:	5f                   	pop    %edi
  80010c:	5d                   	pop    %ebp
  80010d:	c3                   	ret    

0080010e <sys_env_destroy>:

int
sys_env_destroy(envid_t envid)
{
  80010e:	55                   	push   %ebp
  80010f:	89 e5                	mov    %esp,%ebp
  800111:	57                   	push   %edi
  800112:	56                   	push   %esi
  800113:	53                   	push   %ebx
  800114:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  800117:	b9 00 00 00 00       	mov    $0x0,%ecx
  80011c:	b8 03 00 00 00       	mov    $0x3,%eax
  800121:	8b 55 08             	mov    0x8(%ebp),%edx
  800124:	89 cb                	mov    %ecx,%ebx
  800126:	89 cf                	mov    %ecx,%edi
  800128:	89 ce                	mov    %ecx,%esi
  80012a:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  80012c:	85 c0                	test   %eax,%eax
  80012e:	7e 28                	jle    800158 <sys_env_destroy+0x4a>
		panic("syscall %d returned %d (> 0)", num, ret);
  800130:	89 44 24 10          	mov    %eax,0x10(%esp)
  800134:	c7 44 24 0c 03 00 00 	movl   $0x3,0xc(%esp)
  80013b:	00 
  80013c:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  800143:	00 
  800144:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  80014b:	00 
  80014c:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  800153:	e8 ae 02 00 00       	call   800406 <_panic>

int
sys_env_destroy(envid_t envid)
{
	return syscall(SYS_env_destroy, 1, envid, 0, 0, 0, 0);
}
  800158:	83 c4 2c             	add    $0x2c,%esp
  80015b:	5b                   	pop    %ebx
  80015c:	5e                   	pop    %esi
  80015d:	5f                   	pop    %edi
  80015e:	5d                   	pop    %ebp
  80015f:	c3                   	ret    

00800160 <sys_getenvid>:

envid_t
sys_getenvid(void)
{
  800160:	55                   	push   %ebp
  800161:	89 e5                	mov    %esp,%ebp
  800163:	57                   	push   %edi
  800164:	56                   	push   %esi
  800165:	53                   	push   %ebx
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  800166:	ba 00 00 00 00       	mov    $0x0,%edx
  80016b:	b8 02 00 00 00       	mov    $0x2,%eax
  800170:	89 d1                	mov    %edx,%ecx
  800172:	89 d3                	mov    %edx,%ebx
  800174:	89 d7                	mov    %edx,%edi
  800176:	89 d6                	mov    %edx,%esi
  800178:	cd 30                	int    $0x30

envid_t
sys_getenvid(void)
{
	 return syscall(SYS_getenvid, 0, 0, 0, 0, 0, 0);
}
  80017a:	5b                   	pop    %ebx
  80017b:	5e                   	pop    %esi
  80017c:	5f                   	pop    %edi
  80017d:	5d                   	pop    %ebp
  80017e:	c3                   	ret    

0080017f <sys_yield>:

void
sys_yield(void)
{
  80017f:	55                   	push   %ebp
  800180:	89 e5                	mov    %esp,%ebp
  800182:	57                   	push   %edi
  800183:	56                   	push   %esi
  800184:	53                   	push   %ebx
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  800185:	ba 00 00 00 00       	mov    $0x0,%edx
  80018a:	b8 0b 00 00 00       	mov    $0xb,%eax
  80018f:	89 d1                	mov    %edx,%ecx
  800191:	89 d3                	mov    %edx,%ebx
  800193:	89 d7                	mov    %edx,%edi
  800195:	89 d6                	mov    %edx,%esi
  800197:	cd 30                	int    $0x30

void
sys_yield(void)
{
	syscall(SYS_yield, 0, 0, 0, 0, 0, 0);
}
  800199:	5b                   	pop    %ebx
  80019a:	5e                   	pop    %esi
  80019b:	5f                   	pop    %edi
  80019c:	5d                   	pop    %ebp
  80019d:	c3                   	ret    

0080019e <sys_page_alloc>:

int
sys_page_alloc(envid_t envid, void *va, int perm)
{
  80019e:	55                   	push   %ebp
  80019f:	89 e5                	mov    %esp,%ebp
  8001a1:	57                   	push   %edi
  8001a2:	56                   	push   %esi
  8001a3:	53                   	push   %ebx
  8001a4:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8001a7:	be 00 00 00 00       	mov    $0x0,%esi
  8001ac:	b8 04 00 00 00       	mov    $0x4,%eax
  8001b1:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8001b4:	8b 55 08             	mov    0x8(%ebp),%edx
  8001b7:	8b 5d 10             	mov    0x10(%ebp),%ebx
  8001ba:	89 f7                	mov    %esi,%edi
  8001bc:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  8001be:	85 c0                	test   %eax,%eax
  8001c0:	7e 28                	jle    8001ea <sys_page_alloc+0x4c>
		panic("syscall %d returned %d (> 0)", num, ret);
  8001c2:	89 44 24 10          	mov    %eax,0x10(%esp)
  8001c6:	c7 44 24 0c 04 00 00 	movl   $0x4,0xc(%esp)
  8001cd:	00 
  8001ce:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  8001d5:	00 
  8001d6:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  8001dd:	00 
  8001de:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  8001e5:	e8 1c 02 00 00       	call   800406 <_panic>

int
sys_page_alloc(envid_t envid, void *va, int perm)
{
	return syscall(SYS_page_alloc, 1, envid, (uint32_t) va, perm, 0, 0);
}
  8001ea:	83 c4 2c             	add    $0x2c,%esp
  8001ed:	5b                   	pop    %ebx
  8001ee:	5e                   	pop    %esi
  8001ef:	5f                   	pop    %edi
  8001f0:	5d                   	pop    %ebp
  8001f1:	c3                   	ret    

008001f2 <sys_page_map>:

int
sys_page_map(envid_t srcenv, void *srcva, envid_t dstenv, void *dstva, int perm)
{
  8001f2:	55                   	push   %ebp
  8001f3:	89 e5                	mov    %esp,%ebp
  8001f5:	57                   	push   %edi
  8001f6:	56                   	push   %esi
  8001f7:	53                   	push   %ebx
  8001f8:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8001fb:	b8 05 00 00 00       	mov    $0x5,%eax
  800200:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800203:	8b 55 08             	mov    0x8(%ebp),%edx
  800206:	8b 5d 10             	mov    0x10(%ebp),%ebx
  800209:	8b 7d 14             	mov    0x14(%ebp),%edi
  80020c:	8b 75 18             	mov    0x18(%ebp),%esi
  80020f:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  800211:	85 c0                	test   %eax,%eax
  800213:	7e 28                	jle    80023d <sys_page_map+0x4b>
		panic("syscall %d returned %d (> 0)", num, ret);
  800215:	89 44 24 10          	mov    %eax,0x10(%esp)
  800219:	c7 44 24 0c 05 00 00 	movl   $0x5,0xc(%esp)
  800220:	00 
  800221:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  800228:	00 
  800229:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  800230:	00 
  800231:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  800238:	e8 c9 01 00 00       	call   800406 <_panic>

int
sys_page_map(envid_t srcenv, void *srcva, envid_t dstenv, void *dstva, int perm)
{
	return syscall(SYS_page_map, 1, srcenv, (uint32_t) srcva, dstenv, (uint32_t) dstva, perm);
}
  80023d:	83 c4 2c             	add    $0x2c,%esp
  800240:	5b                   	pop    %ebx
  800241:	5e                   	pop    %esi
  800242:	5f                   	pop    %edi
  800243:	5d                   	pop    %ebp
  800244:	c3                   	ret    

00800245 <sys_page_unmap>:

int
sys_page_unmap(envid_t envid, void *va)
{
  800245:	55                   	push   %ebp
  800246:	89 e5                	mov    %esp,%ebp
  800248:	57                   	push   %edi
  800249:	56                   	push   %esi
  80024a:	53                   	push   %ebx
  80024b:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  80024e:	bb 00 00 00 00       	mov    $0x0,%ebx
  800253:	b8 06 00 00 00       	mov    $0x6,%eax
  800258:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  80025b:	8b 55 08             	mov    0x8(%ebp),%edx
  80025e:	89 df                	mov    %ebx,%edi
  800260:	89 de                	mov    %ebx,%esi
  800262:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  800264:	85 c0                	test   %eax,%eax
  800266:	7e 28                	jle    800290 <sys_page_unmap+0x4b>
		panic("syscall %d returned %d (> 0)", num, ret);
  800268:	89 44 24 10          	mov    %eax,0x10(%esp)
  80026c:	c7 44 24 0c 06 00 00 	movl   $0x6,0xc(%esp)
  800273:	00 
  800274:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  80027b:	00 
  80027c:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  800283:	00 
  800284:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  80028b:	e8 76 01 00 00       	call   800406 <_panic>

int
sys_page_unmap(envid_t envid, void *va)
{
	return syscall(SYS_page_unmap, 1, envid, (uint32_t) va, 0, 0, 0);
}
  800290:	83 c4 2c             	add    $0x2c,%esp
  800293:	5b                   	pop    %ebx
  800294:	5e                   	pop    %esi
  800295:	5f                   	pop    %edi
  800296:	5d                   	pop    %ebp
  800297:	c3                   	ret    

00800298 <sys_env_set_status>:

// sys_exofork is inlined in lib.h

int
sys_env_set_status(envid_t envid, int status)
{
  800298:	55                   	push   %ebp
  800299:	89 e5                	mov    %esp,%ebp
  80029b:	57                   	push   %edi
  80029c:	56                   	push   %esi
  80029d:	53                   	push   %ebx
  80029e:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8002a1:	bb 00 00 00 00       	mov    $0x0,%ebx
  8002a6:	b8 08 00 00 00       	mov    $0x8,%eax
  8002ab:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8002ae:	8b 55 08             	mov    0x8(%ebp),%edx
  8002b1:	89 df                	mov    %ebx,%edi
  8002b3:	89 de                	mov    %ebx,%esi
  8002b5:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  8002b7:	85 c0                	test   %eax,%eax
  8002b9:	7e 28                	jle    8002e3 <sys_env_set_status+0x4b>
		panic("syscall %d returned %d (> 0)", num, ret);
  8002bb:	89 44 24 10          	mov    %eax,0x10(%esp)
  8002bf:	c7 44 24 0c 08 00 00 	movl   $0x8,0xc(%esp)
  8002c6:	00 
  8002c7:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  8002ce:	00 
  8002cf:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  8002d6:	00 
  8002d7:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  8002de:	e8 23 01 00 00       	call   800406 <_panic>

int
sys_env_set_status(envid_t envid, int status)
{
	return syscall(SYS_env_set_status, 1, envid, status, 0, 0, 0);
}
  8002e3:	83 c4 2c             	add    $0x2c,%esp
  8002e6:	5b                   	pop    %ebx
  8002e7:	5e                   	pop    %esi
  8002e8:	5f                   	pop    %edi
  8002e9:	5d                   	pop    %ebp
  8002ea:	c3                   	ret    

008002eb <sys_env_set_trapframe>:

int
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
  8002eb:	55                   	push   %ebp
  8002ec:	89 e5                	mov    %esp,%ebp
  8002ee:	57                   	push   %edi
  8002ef:	56                   	push   %esi
  8002f0:	53                   	push   %ebx
  8002f1:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8002f4:	bb 00 00 00 00       	mov    $0x0,%ebx
  8002f9:	b8 09 00 00 00       	mov    $0x9,%eax
  8002fe:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800301:	8b 55 08             	mov    0x8(%ebp),%edx
  800304:	89 df                	mov    %ebx,%edi
  800306:	89 de                	mov    %ebx,%esi
  800308:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  80030a:	85 c0                	test   %eax,%eax
  80030c:	7e 28                	jle    800336 <sys_env_set_trapframe+0x4b>
		panic("syscall %d returned %d (> 0)", num, ret);
  80030e:	89 44 24 10          	mov    %eax,0x10(%esp)
  800312:	c7 44 24 0c 09 00 00 	movl   $0x9,0xc(%esp)
  800319:	00 
  80031a:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  800321:	00 
  800322:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  800329:	00 
  80032a:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  800331:	e8 d0 00 00 00       	call   800406 <_panic>

int
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	return syscall(SYS_env_set_trapframe, 1, envid, (uint32_t) tf, 0, 0, 0);
}
  800336:	83 c4 2c             	add    $0x2c,%esp
  800339:	5b                   	pop    %ebx
  80033a:	5e                   	pop    %esi
  80033b:	5f                   	pop    %edi
  80033c:	5d                   	pop    %ebp
  80033d:	c3                   	ret    

0080033e <sys_env_set_pgfault_upcall>:

int
sys_env_set_pgfault_upcall(envid_t envid, void *upcall)
{
  80033e:	55                   	push   %ebp
  80033f:	89 e5                	mov    %esp,%ebp
  800341:	57                   	push   %edi
  800342:	56                   	push   %esi
  800343:	53                   	push   %ebx
  800344:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  800347:	bb 00 00 00 00       	mov    $0x0,%ebx
  80034c:	b8 0a 00 00 00       	mov    $0xa,%eax
  800351:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800354:	8b 55 08             	mov    0x8(%ebp),%edx
  800357:	89 df                	mov    %ebx,%edi
  800359:	89 de                	mov    %ebx,%esi
  80035b:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  80035d:	85 c0                	test   %eax,%eax
  80035f:	7e 28                	jle    800389 <sys_env_set_pgfault_upcall+0x4b>
		panic("syscall %d returned %d (> 0)", num, ret);
  800361:	89 44 24 10          	mov    %eax,0x10(%esp)
  800365:	c7 44 24 0c 0a 00 00 	movl   $0xa,0xc(%esp)
  80036c:	00 
  80036d:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  800374:	00 
  800375:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  80037c:	00 
  80037d:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  800384:	e8 7d 00 00 00       	call   800406 <_panic>

int
sys_env_set_pgfault_upcall(envid_t envid, void *upcall)
{
	return syscall(SYS_env_set_pgfault_upcall, 1, envid, (uint32_t) upcall, 0, 0, 0);
}
  800389:	83 c4 2c             	add    $0x2c,%esp
  80038c:	5b                   	pop    %ebx
  80038d:	5e                   	pop    %esi
  80038e:	5f                   	pop    %edi
  80038f:	5d                   	pop    %ebp
  800390:	c3                   	ret    

00800391 <sys_ipc_try_send>:

int
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
  800391:	55                   	push   %ebp
  800392:	89 e5                	mov    %esp,%ebp
  800394:	57                   	push   %edi
  800395:	56                   	push   %esi
  800396:	53                   	push   %ebx
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  800397:	be 00 00 00 00       	mov    $0x0,%esi
  80039c:	b8 0c 00 00 00       	mov    $0xc,%eax
  8003a1:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  8003a4:	8b 55 08             	mov    0x8(%ebp),%edx
  8003a7:	8b 5d 10             	mov    0x10(%ebp),%ebx
  8003aa:	8b 7d 14             	mov    0x14(%ebp),%edi
  8003ad:	cd 30                	int    $0x30

int
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, int perm)
{
	return syscall(SYS_ipc_try_send, 0, envid, value, (uint32_t) srcva, perm, 0);
}
  8003af:	5b                   	pop    %ebx
  8003b0:	5e                   	pop    %esi
  8003b1:	5f                   	pop    %edi
  8003b2:	5d                   	pop    %ebp
  8003b3:	c3                   	ret    

008003b4 <sys_ipc_recv>:

int
sys_ipc_recv(void *dstva)
{
  8003b4:	55                   	push   %ebp
  8003b5:	89 e5                	mov    %esp,%ebp
  8003b7:	57                   	push   %edi
  8003b8:	56                   	push   %esi
  8003b9:	53                   	push   %ebx
  8003ba:	83 ec 2c             	sub    $0x2c,%esp
	//
	// The last clause tells the assembler that this can
	// potentially change the condition codes and arbitrary
	// memory locations.

	asm volatile("int %1\n"
  8003bd:	b9 00 00 00 00       	mov    $0x0,%ecx
  8003c2:	b8 0d 00 00 00       	mov    $0xd,%eax
  8003c7:	8b 55 08             	mov    0x8(%ebp),%edx
  8003ca:	89 cb                	mov    %ecx,%ebx
  8003cc:	89 cf                	mov    %ecx,%edi
  8003ce:	89 ce                	mov    %ecx,%esi
  8003d0:	cd 30                	int    $0x30
		  "b" (a3),
		  "D" (a4),
		  "S" (a5)
		: "cc", "memory");

	if(check && ret > 0)
  8003d2:	85 c0                	test   %eax,%eax
  8003d4:	7e 28                	jle    8003fe <sys_ipc_recv+0x4a>
		panic("syscall %d returned %d (> 0)", num, ret);
  8003d6:	89 44 24 10          	mov    %eax,0x10(%esp)
  8003da:	c7 44 24 0c 0d 00 00 	movl   $0xd,0xc(%esp)
  8003e1:	00 
  8003e2:	c7 44 24 08 6a 11 80 	movl   $0x80116a,0x8(%esp)
  8003e9:	00 
  8003ea:	c7 44 24 04 23 00 00 	movl   $0x23,0x4(%esp)
  8003f1:	00 
  8003f2:	c7 04 24 87 11 80 00 	movl   $0x801187,(%esp)
  8003f9:	e8 08 00 00 00       	call   800406 <_panic>

int
sys_ipc_recv(void *dstva)
{
	return syscall(SYS_ipc_recv, 1, (uint32_t)dstva, 0, 0, 0, 0);
}
  8003fe:	83 c4 2c             	add    $0x2c,%esp
  800401:	5b                   	pop    %ebx
  800402:	5e                   	pop    %esi
  800403:	5f                   	pop    %edi
  800404:	5d                   	pop    %ebp
  800405:	c3                   	ret    

00800406 <_panic>:
 * It prints "panic: <message>", then causes a breakpoint exception,
 * which causes JOS to enter the JOS kernel monitor.
 */
void
_panic(const char *file, int line, const char *fmt, ...)
{
  800406:	55                   	push   %ebp
  800407:	89 e5                	mov    %esp,%ebp
  800409:	56                   	push   %esi
  80040a:	53                   	push   %ebx
  80040b:	83 ec 20             	sub    $0x20,%esp
	va_list ap;

	va_start(ap, fmt);
  80040e:	8d 5d 14             	lea    0x14(%ebp),%ebx

	// Print the panic message
	cprintf("[%08x] user panic in %s at %s:%d: ",
  800411:	8b 35 00 20 80 00    	mov    0x802000,%esi
  800417:	e8 44 fd ff ff       	call   800160 <sys_getenvid>
  80041c:	8b 55 0c             	mov    0xc(%ebp),%edx
  80041f:	89 54 24 10          	mov    %edx,0x10(%esp)
  800423:	8b 55 08             	mov    0x8(%ebp),%edx
  800426:	89 54 24 0c          	mov    %edx,0xc(%esp)
  80042a:	89 74 24 08          	mov    %esi,0x8(%esp)
  80042e:	89 44 24 04          	mov    %eax,0x4(%esp)
  800432:	c7 04 24 98 11 80 00 	movl   $0x801198,(%esp)
  800439:	e8 c1 00 00 00       	call   8004ff <cprintf>
		sys_getenvid(), binaryname, file, line);
	vcprintf(fmt, ap);
  80043e:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  800442:	8b 45 10             	mov    0x10(%ebp),%eax
  800445:	89 04 24             	mov    %eax,(%esp)
  800448:	e8 51 00 00 00       	call   80049e <vcprintf>
	cprintf("\n");
  80044d:	c7 04 24 bb 11 80 00 	movl   $0x8011bb,(%esp)
  800454:	e8 a6 00 00 00       	call   8004ff <cprintf>

	// Cause a breakpoint exception
	while (1)
		asm volatile("int3");
  800459:	cc                   	int3   
  80045a:	eb fd                	jmp    800459 <_panic+0x53>

0080045c <putch>:
};


static void
putch(int ch, struct printbuf *b)
{
  80045c:	55                   	push   %ebp
  80045d:	89 e5                	mov    %esp,%ebp
  80045f:	53                   	push   %ebx
  800460:	83 ec 14             	sub    $0x14,%esp
  800463:	8b 5d 0c             	mov    0xc(%ebp),%ebx
	b->buf[b->idx++] = ch;
  800466:	8b 13                	mov    (%ebx),%edx
  800468:	8d 42 01             	lea    0x1(%edx),%eax
  80046b:	89 03                	mov    %eax,(%ebx)
  80046d:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800470:	88 4c 13 08          	mov    %cl,0x8(%ebx,%edx,1)
	if (b->idx == 256-1) {
  800474:	3d ff 00 00 00       	cmp    $0xff,%eax
  800479:	75 19                	jne    800494 <putch+0x38>
		sys_cputs(b->buf, b->idx);
  80047b:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
  800482:	00 
  800483:	8d 43 08             	lea    0x8(%ebx),%eax
  800486:	89 04 24             	mov    %eax,(%esp)
  800489:	e8 43 fc ff ff       	call   8000d1 <sys_cputs>
		b->idx = 0;
  80048e:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
	}
	b->cnt++;
  800494:	83 43 04 01          	addl   $0x1,0x4(%ebx)
}
  800498:	83 c4 14             	add    $0x14,%esp
  80049b:	5b                   	pop    %ebx
  80049c:	5d                   	pop    %ebp
  80049d:	c3                   	ret    

0080049e <vcprintf>:

int
vcprintf(const char *fmt, va_list ap)
{
  80049e:	55                   	push   %ebp
  80049f:	89 e5                	mov    %esp,%ebp
  8004a1:	81 ec 28 01 00 00    	sub    $0x128,%esp
	struct printbuf b;

	b.idx = 0;
  8004a7:	c7 85 f0 fe ff ff 00 	movl   $0x0,-0x110(%ebp)
  8004ae:	00 00 00 
	b.cnt = 0;
  8004b1:	c7 85 f4 fe ff ff 00 	movl   $0x0,-0x10c(%ebp)
  8004b8:	00 00 00 
	vprintfmt((void*)putch, &b, fmt, ap);
  8004bb:	8b 45 0c             	mov    0xc(%ebp),%eax
  8004be:	89 44 24 0c          	mov    %eax,0xc(%esp)
  8004c2:	8b 45 08             	mov    0x8(%ebp),%eax
  8004c5:	89 44 24 08          	mov    %eax,0x8(%esp)
  8004c9:	8d 85 f0 fe ff ff    	lea    -0x110(%ebp),%eax
  8004cf:	89 44 24 04          	mov    %eax,0x4(%esp)
  8004d3:	c7 04 24 5c 04 80 00 	movl   $0x80045c,(%esp)
  8004da:	e8 75 01 00 00       	call   800654 <vprintfmt>
	sys_cputs(b.buf, b.idx);
  8004df:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
  8004e5:	89 44 24 04          	mov    %eax,0x4(%esp)
  8004e9:	8d 85 f8 fe ff ff    	lea    -0x108(%ebp),%eax
  8004ef:	89 04 24             	mov    %eax,(%esp)
  8004f2:	e8 da fb ff ff       	call   8000d1 <sys_cputs>

	return b.cnt;
}
  8004f7:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
  8004fd:	c9                   	leave  
  8004fe:	c3                   	ret    

008004ff <cprintf>:

int
cprintf(const char *fmt, ...)
{
  8004ff:	55                   	push   %ebp
  800500:	89 e5                	mov    %esp,%ebp
  800502:	83 ec 18             	sub    $0x18,%esp
	va_list ap;
	int cnt;

	va_start(ap, fmt);
  800505:	8d 45 0c             	lea    0xc(%ebp),%eax
	cnt = vcprintf(fmt, ap);
  800508:	89 44 24 04          	mov    %eax,0x4(%esp)
  80050c:	8b 45 08             	mov    0x8(%ebp),%eax
  80050f:	89 04 24             	mov    %eax,(%esp)
  800512:	e8 87 ff ff ff       	call   80049e <vcprintf>
	va_end(ap);

	return cnt;
}
  800517:	c9                   	leave  
  800518:	c3                   	ret    
  800519:	66 90                	xchg   %ax,%ax
  80051b:	66 90                	xchg   %ax,%ax
  80051d:	66 90                	xchg   %ax,%ax
  80051f:	90                   	nop

00800520 <printnum>:
 * using specified putch function and associated pointer putdat.
 */
static void
printnum(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int width, int padc)
{
  800520:	55                   	push   %ebp
  800521:	89 e5                	mov    %esp,%ebp
  800523:	57                   	push   %edi
  800524:	56                   	push   %esi
  800525:	53                   	push   %ebx
  800526:	83 ec 3c             	sub    $0x3c,%esp
  800529:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  80052c:	89 d7                	mov    %edx,%edi
  80052e:	8b 45 08             	mov    0x8(%ebp),%eax
  800531:	89 45 e0             	mov    %eax,-0x20(%ebp)
  800534:	8b 45 0c             	mov    0xc(%ebp),%eax
  800537:	89 c3                	mov    %eax,%ebx
  800539:	89 45 d4             	mov    %eax,-0x2c(%ebp)
  80053c:	8b 45 10             	mov    0x10(%ebp),%eax
  80053f:	8b 75 14             	mov    0x14(%ebp),%esi
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
  800542:	b9 00 00 00 00       	mov    $0x0,%ecx
  800547:	89 45 d8             	mov    %eax,-0x28(%ebp)
  80054a:	89 4d dc             	mov    %ecx,-0x24(%ebp)
  80054d:	39 d9                	cmp    %ebx,%ecx
  80054f:	72 05                	jb     800556 <printnum+0x36>
  800551:	3b 45 e0             	cmp    -0x20(%ebp),%eax
  800554:	77 69                	ja     8005bf <printnum+0x9f>
		printnum(putch, putdat, num / base, base, width - 1, padc);
  800556:	8b 4d 18             	mov    0x18(%ebp),%ecx
  800559:	89 4c 24 10          	mov    %ecx,0x10(%esp)
  80055d:	83 ee 01             	sub    $0x1,%esi
  800560:	89 74 24 0c          	mov    %esi,0xc(%esp)
  800564:	89 44 24 08          	mov    %eax,0x8(%esp)
  800568:	8b 44 24 08          	mov    0x8(%esp),%eax
  80056c:	8b 54 24 0c          	mov    0xc(%esp),%edx
  800570:	89 c3                	mov    %eax,%ebx
  800572:	89 d6                	mov    %edx,%esi
  800574:	8b 55 d8             	mov    -0x28(%ebp),%edx
  800577:	8b 4d dc             	mov    -0x24(%ebp),%ecx
  80057a:	89 54 24 08          	mov    %edx,0x8(%esp)
  80057e:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
  800582:	8b 45 e0             	mov    -0x20(%ebp),%eax
  800585:	89 04 24             	mov    %eax,(%esp)
  800588:	8b 45 d4             	mov    -0x2c(%ebp),%eax
  80058b:	89 44 24 04          	mov    %eax,0x4(%esp)
  80058f:	e8 3c 09 00 00       	call   800ed0 <__udivdi3>
  800594:	89 d9                	mov    %ebx,%ecx
  800596:	89 4c 24 08          	mov    %ecx,0x8(%esp)
  80059a:	89 74 24 0c          	mov    %esi,0xc(%esp)
  80059e:	89 04 24             	mov    %eax,(%esp)
  8005a1:	89 54 24 04          	mov    %edx,0x4(%esp)
  8005a5:	89 fa                	mov    %edi,%edx
  8005a7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  8005aa:	e8 71 ff ff ff       	call   800520 <printnum>
  8005af:	eb 1b                	jmp    8005cc <printnum+0xac>
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
			putch(padc, putdat);
  8005b1:	89 7c 24 04          	mov    %edi,0x4(%esp)
  8005b5:	8b 45 18             	mov    0x18(%ebp),%eax
  8005b8:	89 04 24             	mov    %eax,(%esp)
  8005bb:	ff d3                	call   *%ebx
  8005bd:	eb 03                	jmp    8005c2 <printnum+0xa2>
  8005bf:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
	// first recursively print all preceding (more significant) digits
	if (num >= base) {
		printnum(putch, putdat, num / base, base, width - 1, padc);
	} else {
		// print any needed pad characters before first digit
		while (--width > 0)
  8005c2:	83 ee 01             	sub    $0x1,%esi
  8005c5:	85 f6                	test   %esi,%esi
  8005c7:	7f e8                	jg     8005b1 <printnum+0x91>
  8005c9:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
			putch(padc, putdat);
	}

	// then print this (the least significant) digit
	putch("0123456789abcdef"[num % base], putdat);
  8005cc:	89 7c 24 04          	mov    %edi,0x4(%esp)
  8005d0:	8b 7c 24 04          	mov    0x4(%esp),%edi
  8005d4:	8b 45 d8             	mov    -0x28(%ebp),%eax
  8005d7:	8b 55 dc             	mov    -0x24(%ebp),%edx
  8005da:	89 44 24 08          	mov    %eax,0x8(%esp)
  8005de:	89 54 24 0c          	mov    %edx,0xc(%esp)
  8005e2:	8b 45 e0             	mov    -0x20(%ebp),%eax
  8005e5:	89 04 24             	mov    %eax,(%esp)
  8005e8:	8b 45 d4             	mov    -0x2c(%ebp),%eax
  8005eb:	89 44 24 04          	mov    %eax,0x4(%esp)
  8005ef:	e8 0c 0a 00 00       	call   801000 <__umoddi3>
  8005f4:	89 7c 24 04          	mov    %edi,0x4(%esp)
  8005f8:	0f be 80 bd 11 80 00 	movsbl 0x8011bd(%eax),%eax
  8005ff:	89 04 24             	mov    %eax,(%esp)
  800602:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  800605:	ff d0                	call   *%eax
}
  800607:	83 c4 3c             	add    $0x3c,%esp
  80060a:	5b                   	pop    %ebx
  80060b:	5e                   	pop    %esi
  80060c:	5f                   	pop    %edi
  80060d:	5d                   	pop    %ebp
  80060e:	c3                   	ret    

0080060f <sprintputch>:
	int cnt;
};

static void
sprintputch(int ch, struct sprintbuf *b)
{
  80060f:	55                   	push   %ebp
  800610:	89 e5                	mov    %esp,%ebp
  800612:	8b 45 0c             	mov    0xc(%ebp),%eax
	b->cnt++;
  800615:	83 40 08 01          	addl   $0x1,0x8(%eax)
	if (b->buf < b->ebuf)
  800619:	8b 10                	mov    (%eax),%edx
  80061b:	3b 50 04             	cmp    0x4(%eax),%edx
  80061e:	73 0a                	jae    80062a <sprintputch+0x1b>
		*b->buf++ = ch;
  800620:	8d 4a 01             	lea    0x1(%edx),%ecx
  800623:	89 08                	mov    %ecx,(%eax)
  800625:	8b 45 08             	mov    0x8(%ebp),%eax
  800628:	88 02                	mov    %al,(%edx)
}
  80062a:	5d                   	pop    %ebp
  80062b:	c3                   	ret    

0080062c <printfmt>:
	}
}

void
printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...)
{
  80062c:	55                   	push   %ebp
  80062d:	89 e5                	mov    %esp,%ebp
  80062f:	83 ec 18             	sub    $0x18,%esp
	va_list ap;

	va_start(ap, fmt);
  800632:	8d 45 14             	lea    0x14(%ebp),%eax
	vprintfmt(putch, putdat, fmt, ap);
  800635:	89 44 24 0c          	mov    %eax,0xc(%esp)
  800639:	8b 45 10             	mov    0x10(%ebp),%eax
  80063c:	89 44 24 08          	mov    %eax,0x8(%esp)
  800640:	8b 45 0c             	mov    0xc(%ebp),%eax
  800643:	89 44 24 04          	mov    %eax,0x4(%esp)
  800647:	8b 45 08             	mov    0x8(%ebp),%eax
  80064a:	89 04 24             	mov    %eax,(%esp)
  80064d:	e8 02 00 00 00       	call   800654 <vprintfmt>
	va_end(ap);
}
  800652:	c9                   	leave  
  800653:	c3                   	ret    

00800654 <vprintfmt>:
// Main function to format and print a string.
void printfmt(void (*putch)(int, void*), void *putdat, const char *fmt, ...);

void
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
  800654:	55                   	push   %ebp
  800655:	89 e5                	mov    %esp,%ebp
  800657:	57                   	push   %edi
  800658:	56                   	push   %esi
  800659:	53                   	push   %ebx
  80065a:	83 ec 3c             	sub    $0x3c,%esp
  80065d:	8b 75 08             	mov    0x8(%ebp),%esi
  800660:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  800663:	8b 7d 10             	mov    0x10(%ebp),%edi
  800666:	eb 11                	jmp    800679 <vprintfmt+0x25>
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
			if (ch == '\0')
  800668:	85 c0                	test   %eax,%eax
  80066a:	0f 84 48 04 00 00    	je     800ab8 <vprintfmt+0x464>
				return;
			putch(ch, putdat);
  800670:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  800674:	89 04 24             	mov    %eax,(%esp)
  800677:	ff d6                	call   *%esi
	unsigned long long num;
	int base, lflag, width, precision, altflag;
	char padc;

	while (1) {
		while ((ch = *(unsigned char *) fmt++) != '%') {
  800679:	83 c7 01             	add    $0x1,%edi
  80067c:	0f b6 47 ff          	movzbl -0x1(%edi),%eax
  800680:	83 f8 25             	cmp    $0x25,%eax
  800683:	75 e3                	jne    800668 <vprintfmt+0x14>
  800685:	c6 45 d4 20          	movb   $0x20,-0x2c(%ebp)
  800689:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)
  800690:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
  800697:	c7 45 e0 ff ff ff ff 	movl   $0xffffffff,-0x20(%ebp)
  80069e:	b9 00 00 00 00       	mov    $0x0,%ecx
  8006a3:	eb 1f                	jmp    8006c4 <vprintfmt+0x70>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8006a5:	8b 7d e4             	mov    -0x1c(%ebp),%edi

		// flag to pad on the right
		case '-':
			padc = '-';
  8006a8:	c6 45 d4 2d          	movb   $0x2d,-0x2c(%ebp)
  8006ac:	eb 16                	jmp    8006c4 <vprintfmt+0x70>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8006ae:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			padc = '-';
			goto reswitch;

		// flag to pad with 0's instead of spaces
		case '0':
			padc = '0';
  8006b1:	c6 45 d4 30          	movb   $0x30,-0x2c(%ebp)
  8006b5:	eb 0d                	jmp    8006c4 <vprintfmt+0x70>
			altflag = 1;
			goto reswitch;

		process_precision:
			if (width < 0)
				width = precision, precision = -1;
  8006b7:	8b 45 d0             	mov    -0x30(%ebp),%eax
  8006ba:	89 45 e0             	mov    %eax,-0x20(%ebp)
  8006bd:	c7 45 d0 ff ff ff ff 	movl   $0xffffffff,-0x30(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8006c4:	8d 47 01             	lea    0x1(%edi),%eax
  8006c7:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  8006ca:	0f b6 17             	movzbl (%edi),%edx
  8006cd:	0f b6 c2             	movzbl %dl,%eax
  8006d0:	83 ea 23             	sub    $0x23,%edx
  8006d3:	80 fa 55             	cmp    $0x55,%dl
  8006d6:	0f 87 bf 03 00 00    	ja     800a9b <vprintfmt+0x447>
  8006dc:	0f b6 d2             	movzbl %dl,%edx
  8006df:	ff 24 95 00 13 80 00 	jmp    *0x801300(,%edx,4)
  8006e6:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  8006e9:	ba 00 00 00 00       	mov    $0x0,%edx
  8006ee:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
				precision = precision * 10 + ch - '0';
  8006f1:	8d 14 92             	lea    (%edx,%edx,4),%edx
  8006f4:	8d 54 50 d0          	lea    -0x30(%eax,%edx,2),%edx
				ch = *fmt;
  8006f8:	0f be 07             	movsbl (%edi),%eax
				if (ch < '0' || ch > '9')
  8006fb:	8d 48 d0             	lea    -0x30(%eax),%ecx
  8006fe:	83 f9 09             	cmp    $0x9,%ecx
  800701:	77 3c                	ja     80073f <vprintfmt+0xeb>
		case '5':
		case '6':
		case '7':
		case '8':
		case '9':
			for (precision = 0; ; ++fmt) {
  800703:	83 c7 01             	add    $0x1,%edi
				precision = precision * 10 + ch - '0';
				ch = *fmt;
				if (ch < '0' || ch > '9')
					break;
			}
  800706:	eb e9                	jmp    8006f1 <vprintfmt+0x9d>
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
  800708:	8b 45 14             	mov    0x14(%ebp),%eax
  80070b:	8b 00                	mov    (%eax),%eax
  80070d:	89 45 d0             	mov    %eax,-0x30(%ebp)
  800710:	8b 45 14             	mov    0x14(%ebp),%eax
  800713:	8d 40 04             	lea    0x4(%eax),%eax
  800716:	89 45 14             	mov    %eax,0x14(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800719:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			}
			goto process_precision;

		case '*':
			precision = va_arg(ap, int);
			goto process_precision;
  80071c:	eb 27                	jmp    800745 <vprintfmt+0xf1>
  80071e:	8b 55 e0             	mov    -0x20(%ebp),%edx
  800721:	85 d2                	test   %edx,%edx
  800723:	b8 00 00 00 00       	mov    $0x0,%eax
  800728:	0f 49 c2             	cmovns %edx,%eax
  80072b:	89 45 e0             	mov    %eax,-0x20(%ebp)
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  80072e:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  800731:	eb 91                	jmp    8006c4 <vprintfmt+0x70>
  800733:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			if (width < 0)
				width = 0;
			goto reswitch;

		case '#':
			altflag = 1;
  800736:	c7 45 d8 01 00 00 00 	movl   $0x1,-0x28(%ebp)
			goto reswitch;
  80073d:	eb 85                	jmp    8006c4 <vprintfmt+0x70>
  80073f:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
  800742:	89 55 d0             	mov    %edx,-0x30(%ebp)

		process_precision:
			if (width < 0)
  800745:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
  800749:	0f 89 75 ff ff ff    	jns    8006c4 <vprintfmt+0x70>
  80074f:	e9 63 ff ff ff       	jmp    8006b7 <vprintfmt+0x63>
				width = precision, precision = -1;
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
  800754:	83 c1 01             	add    $0x1,%ecx
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800757:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// long flag (doubled for long long)
		case 'l':
			lflag++;
			goto reswitch;
  80075a:	e9 65 ff ff ff       	jmp    8006c4 <vprintfmt+0x70>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  80075f:	8b 45 14             	mov    0x14(%ebp),%eax
			lflag++;
			goto reswitch;

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
  800762:	83 45 14 04          	addl   $0x4,0x14(%ebp)
  800766:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  80076a:	8b 00                	mov    (%eax),%eax
  80076c:	89 04 24             	mov    %eax,(%esp)
  80076f:	ff d6                	call   *%esi
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800771:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			goto reswitch;

		// character
		case 'c':
			putch(va_arg(ap, int), putdat);
			break;
  800774:	e9 00 ff ff ff       	jmp    800679 <vprintfmt+0x25>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800779:	8b 45 14             	mov    0x14(%ebp),%eax
			putch(va_arg(ap, int), putdat);
			break;

		// error message
		case 'e':
			err = va_arg(ap, int);
  80077c:	83 45 14 04          	addl   $0x4,0x14(%ebp)
  800780:	8b 00                	mov    (%eax),%eax
  800782:	99                   	cltd   
  800783:	31 d0                	xor    %edx,%eax
  800785:	29 d0                	sub    %edx,%eax
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
  800787:	83 f8 0f             	cmp    $0xf,%eax
  80078a:	7f 0b                	jg     800797 <vprintfmt+0x143>
  80078c:	8b 14 85 60 14 80 00 	mov    0x801460(,%eax,4),%edx
  800793:	85 d2                	test   %edx,%edx
  800795:	75 20                	jne    8007b7 <vprintfmt+0x163>
				printfmt(putch, putdat, "error %d", err);
  800797:	89 44 24 0c          	mov    %eax,0xc(%esp)
  80079b:	c7 44 24 08 d5 11 80 	movl   $0x8011d5,0x8(%esp)
  8007a2:	00 
  8007a3:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8007a7:	89 34 24             	mov    %esi,(%esp)
  8007aa:	e8 7d fe ff ff       	call   80062c <printfmt>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8007af:	8b 7d e4             	mov    -0x1c(%ebp),%edi
		case 'e':
			err = va_arg(ap, int);
			if (err < 0)
				err = -err;
			if (err >= MAXERROR || (p = error_string[err]) == NULL)
				printfmt(putch, putdat, "error %d", err);
  8007b2:	e9 c2 fe ff ff       	jmp    800679 <vprintfmt+0x25>
			else
				printfmt(putch, putdat, "%s", p);
  8007b7:	89 54 24 0c          	mov    %edx,0xc(%esp)
  8007bb:	c7 44 24 08 de 11 80 	movl   $0x8011de,0x8(%esp)
  8007c2:	00 
  8007c3:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8007c7:	89 34 24             	mov    %esi,(%esp)
  8007ca:	e8 5d fe ff ff       	call   80062c <printfmt>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8007cf:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  8007d2:	e9 a2 fe ff ff       	jmp    800679 <vprintfmt+0x25>
  8007d7:	8b 45 14             	mov    0x14(%ebp),%eax
  8007da:	8b 55 d0             	mov    -0x30(%ebp),%edx
  8007dd:	8b 4d e0             	mov    -0x20(%ebp),%ecx
  8007e0:	89 4d cc             	mov    %ecx,-0x34(%ebp)
				printfmt(putch, putdat, "%s", p);
			break;

		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
  8007e3:	83 45 14 04          	addl   $0x4,0x14(%ebp)
  8007e7:	8b 38                	mov    (%eax),%edi
				p = "(null)";
  8007e9:	85 ff                	test   %edi,%edi
  8007eb:	b8 ce 11 80 00       	mov    $0x8011ce,%eax
  8007f0:	0f 44 f8             	cmove  %eax,%edi
			if (width > 0 && padc != '-')
  8007f3:	80 7d d4 2d          	cmpb   $0x2d,-0x2c(%ebp)
  8007f7:	0f 84 92 00 00 00    	je     80088f <vprintfmt+0x23b>
  8007fd:	85 c9                	test   %ecx,%ecx
  8007ff:	0f 8e 98 00 00 00    	jle    80089d <vprintfmt+0x249>
				for (width -= strnlen(p, precision); width > 0; width--)
  800805:	89 54 24 04          	mov    %edx,0x4(%esp)
  800809:	89 3c 24             	mov    %edi,(%esp)
  80080c:	e8 47 03 00 00       	call   800b58 <strnlen>
  800811:	8b 4d cc             	mov    -0x34(%ebp),%ecx
  800814:	29 c1                	sub    %eax,%ecx
  800816:	89 4d cc             	mov    %ecx,-0x34(%ebp)
					putch(padc, putdat);
  800819:	0f be 45 d4          	movsbl -0x2c(%ebp),%eax
  80081d:	89 45 e0             	mov    %eax,-0x20(%ebp)
  800820:	89 7d d4             	mov    %edi,-0x2c(%ebp)
  800823:	89 cf                	mov    %ecx,%edi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
  800825:	eb 0f                	jmp    800836 <vprintfmt+0x1e2>
					putch(padc, putdat);
  800827:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  80082b:	8b 45 e0             	mov    -0x20(%ebp),%eax
  80082e:	89 04 24             	mov    %eax,(%esp)
  800831:	ff d6                	call   *%esi
		// string
		case 's':
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
  800833:	83 ef 01             	sub    $0x1,%edi
  800836:	85 ff                	test   %edi,%edi
  800838:	7f ed                	jg     800827 <vprintfmt+0x1d3>
  80083a:	8b 7d d4             	mov    -0x2c(%ebp),%edi
  80083d:	8b 4d cc             	mov    -0x34(%ebp),%ecx
  800840:	85 c9                	test   %ecx,%ecx
  800842:	b8 00 00 00 00       	mov    $0x0,%eax
  800847:	0f 49 c1             	cmovns %ecx,%eax
  80084a:	29 c1                	sub    %eax,%ecx
  80084c:	89 75 08             	mov    %esi,0x8(%ebp)
  80084f:	8b 75 d0             	mov    -0x30(%ebp),%esi
  800852:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  800855:	89 cb                	mov    %ecx,%ebx
  800857:	eb 50                	jmp    8008a9 <vprintfmt+0x255>
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
  800859:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
  80085d:	74 1e                	je     80087d <vprintfmt+0x229>
  80085f:	0f be d2             	movsbl %dl,%edx
  800862:	83 ea 20             	sub    $0x20,%edx
  800865:	83 fa 5e             	cmp    $0x5e,%edx
  800868:	76 13                	jbe    80087d <vprintfmt+0x229>
					putch('?', putdat);
  80086a:	8b 45 0c             	mov    0xc(%ebp),%eax
  80086d:	89 44 24 04          	mov    %eax,0x4(%esp)
  800871:	c7 04 24 3f 00 00 00 	movl   $0x3f,(%esp)
  800878:	ff 55 08             	call   *0x8(%ebp)
  80087b:	eb 0d                	jmp    80088a <vprintfmt+0x236>
				else
					putch(ch, putdat);
  80087d:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800880:	89 4c 24 04          	mov    %ecx,0x4(%esp)
  800884:	89 04 24             	mov    %eax,(%esp)
  800887:	ff 55 08             	call   *0x8(%ebp)
			if ((p = va_arg(ap, char *)) == NULL)
				p = "(null)";
			if (width > 0 && padc != '-')
				for (width -= strnlen(p, precision); width > 0; width--)
					putch(padc, putdat);
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
  80088a:	83 eb 01             	sub    $0x1,%ebx
  80088d:	eb 1a                	jmp    8008a9 <vprintfmt+0x255>
  80088f:	89 75 08             	mov    %esi,0x8(%ebp)
  800892:	8b 75 d0             	mov    -0x30(%ebp),%esi
  800895:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  800898:	8b 5d e0             	mov    -0x20(%ebp),%ebx
  80089b:	eb 0c                	jmp    8008a9 <vprintfmt+0x255>
  80089d:	89 75 08             	mov    %esi,0x8(%ebp)
  8008a0:	8b 75 d0             	mov    -0x30(%ebp),%esi
  8008a3:	89 5d 0c             	mov    %ebx,0xc(%ebp)
  8008a6:	8b 5d e0             	mov    -0x20(%ebp),%ebx
  8008a9:	83 c7 01             	add    $0x1,%edi
  8008ac:	0f b6 57 ff          	movzbl -0x1(%edi),%edx
  8008b0:	0f be c2             	movsbl %dl,%eax
  8008b3:	85 c0                	test   %eax,%eax
  8008b5:	74 25                	je     8008dc <vprintfmt+0x288>
  8008b7:	85 f6                	test   %esi,%esi
  8008b9:	78 9e                	js     800859 <vprintfmt+0x205>
  8008bb:	83 ee 01             	sub    $0x1,%esi
  8008be:	79 99                	jns    800859 <vprintfmt+0x205>
  8008c0:	89 df                	mov    %ebx,%edi
  8008c2:	8b 75 08             	mov    0x8(%ebp),%esi
  8008c5:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  8008c8:	eb 1a                	jmp    8008e4 <vprintfmt+0x290>
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
				putch(' ', putdat);
  8008ca:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8008ce:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
  8008d5:	ff d6                	call   *%esi
			for (; (ch = *p++) != '\0' && (precision < 0 || --precision >= 0); width--)
				if (altflag && (ch < ' ' || ch > '~'))
					putch('?', putdat);
				else
					putch(ch, putdat);
			for (; width > 0; width--)
  8008d7:	83 ef 01             	sub    $0x1,%edi
  8008da:	eb 08                	jmp    8008e4 <vprintfmt+0x290>
  8008dc:	89 df                	mov    %ebx,%edi
  8008de:	8b 75 08             	mov    0x8(%ebp),%esi
  8008e1:	8b 5d 0c             	mov    0xc(%ebp),%ebx
  8008e4:	85 ff                	test   %edi,%edi
  8008e6:	7f e2                	jg     8008ca <vprintfmt+0x276>
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8008e8:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  8008eb:	e9 89 fd ff ff       	jmp    800679 <vprintfmt+0x25>
// Same as getuint but signed - can't use getuint
// because of sign extension
static long long
getint(va_list *ap, int lflag)
{
	if (lflag >= 2)
  8008f0:	83 f9 01             	cmp    $0x1,%ecx
  8008f3:	7e 19                	jle    80090e <vprintfmt+0x2ba>
		return va_arg(*ap, long long);
  8008f5:	8b 45 14             	mov    0x14(%ebp),%eax
  8008f8:	8b 50 04             	mov    0x4(%eax),%edx
  8008fb:	8b 00                	mov    (%eax),%eax
  8008fd:	89 45 d8             	mov    %eax,-0x28(%ebp)
  800900:	89 55 dc             	mov    %edx,-0x24(%ebp)
  800903:	8b 45 14             	mov    0x14(%ebp),%eax
  800906:	8d 40 08             	lea    0x8(%eax),%eax
  800909:	89 45 14             	mov    %eax,0x14(%ebp)
  80090c:	eb 38                	jmp    800946 <vprintfmt+0x2f2>
	else if (lflag)
  80090e:	85 c9                	test   %ecx,%ecx
  800910:	74 1b                	je     80092d <vprintfmt+0x2d9>
		return va_arg(*ap, long);
  800912:	8b 45 14             	mov    0x14(%ebp),%eax
  800915:	8b 00                	mov    (%eax),%eax
  800917:	89 45 d8             	mov    %eax,-0x28(%ebp)
  80091a:	89 c1                	mov    %eax,%ecx
  80091c:	c1 f9 1f             	sar    $0x1f,%ecx
  80091f:	89 4d dc             	mov    %ecx,-0x24(%ebp)
  800922:	8b 45 14             	mov    0x14(%ebp),%eax
  800925:	8d 40 04             	lea    0x4(%eax),%eax
  800928:	89 45 14             	mov    %eax,0x14(%ebp)
  80092b:	eb 19                	jmp    800946 <vprintfmt+0x2f2>
	else
		return va_arg(*ap, int);
  80092d:	8b 45 14             	mov    0x14(%ebp),%eax
  800930:	8b 00                	mov    (%eax),%eax
  800932:	89 45 d8             	mov    %eax,-0x28(%ebp)
  800935:	89 c1                	mov    %eax,%ecx
  800937:	c1 f9 1f             	sar    $0x1f,%ecx
  80093a:	89 4d dc             	mov    %ecx,-0x24(%ebp)
  80093d:	8b 45 14             	mov    0x14(%ebp),%eax
  800940:	8d 40 04             	lea    0x4(%eax),%eax
  800943:	89 45 14             	mov    %eax,0x14(%ebp)
				putch(' ', putdat);
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
  800946:	8b 55 d8             	mov    -0x28(%ebp),%edx
  800949:	8b 4d dc             	mov    -0x24(%ebp),%ecx
			if ((long long) num < 0) {
				putch('-', putdat);
				num = -(long long) num;
			}
			base = 10;
  80094c:	bf 0a 00 00 00       	mov    $0xa,%edi
			break;

		// (signed) decimal
		case 'd':
			num = getint(&ap, lflag);
			if ((long long) num < 0) {
  800951:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
  800955:	0f 89 04 01 00 00    	jns    800a5f <vprintfmt+0x40b>
				putch('-', putdat);
  80095b:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  80095f:	c7 04 24 2d 00 00 00 	movl   $0x2d,(%esp)
  800966:	ff d6                	call   *%esi
				num = -(long long) num;
  800968:	8b 55 d8             	mov    -0x28(%ebp),%edx
  80096b:	8b 4d dc             	mov    -0x24(%ebp),%ecx
  80096e:	f7 da                	neg    %edx
  800970:	83 d1 00             	adc    $0x0,%ecx
  800973:	f7 d9                	neg    %ecx
  800975:	e9 e5 00 00 00       	jmp    800a5f <vprintfmt+0x40b>
// Get an unsigned int of various possible sizes from a varargs list,
// depending on the lflag parameter.
static unsigned long long
getuint(va_list *ap, int lflag)
{
	if (lflag >= 2)
  80097a:	83 f9 01             	cmp    $0x1,%ecx
  80097d:	7e 10                	jle    80098f <vprintfmt+0x33b>
		return va_arg(*ap, unsigned long long);
  80097f:	8b 45 14             	mov    0x14(%ebp),%eax
  800982:	8b 10                	mov    (%eax),%edx
  800984:	8b 48 04             	mov    0x4(%eax),%ecx
  800987:	8d 40 08             	lea    0x8(%eax),%eax
  80098a:	89 45 14             	mov    %eax,0x14(%ebp)
  80098d:	eb 26                	jmp    8009b5 <vprintfmt+0x361>
	else if (lflag)
  80098f:	85 c9                	test   %ecx,%ecx
  800991:	74 12                	je     8009a5 <vprintfmt+0x351>
		return va_arg(*ap, unsigned long);
  800993:	8b 45 14             	mov    0x14(%ebp),%eax
  800996:	8b 10                	mov    (%eax),%edx
  800998:	b9 00 00 00 00       	mov    $0x0,%ecx
  80099d:	8d 40 04             	lea    0x4(%eax),%eax
  8009a0:	89 45 14             	mov    %eax,0x14(%ebp)
  8009a3:	eb 10                	jmp    8009b5 <vprintfmt+0x361>
	else
		return va_arg(*ap, unsigned int);
  8009a5:	8b 45 14             	mov    0x14(%ebp),%eax
  8009a8:	8b 10                	mov    (%eax),%edx
  8009aa:	b9 00 00 00 00       	mov    $0x0,%ecx
  8009af:	8d 40 04             	lea    0x4(%eax),%eax
  8009b2:	89 45 14             	mov    %eax,0x14(%ebp)
			goto number;

		// unsigned decimal
		case 'u':
			num = getuint(&ap, lflag);
			base = 10;
  8009b5:	bf 0a 00 00 00       	mov    $0xa,%edi
			goto number;
  8009ba:	e9 a0 00 00 00       	jmp    800a5f <vprintfmt+0x40b>

		// (unsigned) octal
		case 'o':
			// Replace this with your code.
			putch('X', putdat);
  8009bf:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8009c3:	c7 04 24 58 00 00 00 	movl   $0x58,(%esp)
  8009ca:	ff d6                	call   *%esi
			putch('X', putdat);
  8009cc:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8009d0:	c7 04 24 58 00 00 00 	movl   $0x58,(%esp)
  8009d7:	ff d6                	call   *%esi
			putch('X', putdat);
  8009d9:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8009dd:	c7 04 24 58 00 00 00 	movl   $0x58,(%esp)
  8009e4:	ff d6                	call   *%esi
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  8009e6:	8b 7d e4             	mov    -0x1c(%ebp),%edi
		case 'o':
			// Replace this with your code.
			putch('X', putdat);
			putch('X', putdat);
			putch('X', putdat);
			break;
  8009e9:	e9 8b fc ff ff       	jmp    800679 <vprintfmt+0x25>

		// pointer
		case 'p':
			putch('0', putdat);
  8009ee:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8009f2:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
  8009f9:	ff d6                	call   *%esi
			putch('x', putdat);
  8009fb:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  8009ff:	c7 04 24 78 00 00 00 	movl   $0x78,(%esp)
  800a06:	ff d6                	call   *%esi
			num = (unsigned long long)
  800a08:	8b 45 14             	mov    0x14(%ebp),%eax
  800a0b:	8b 10                	mov    (%eax),%edx
  800a0d:	b9 00 00 00 00       	mov    $0x0,%ecx
				(uintptr_t) va_arg(ap, void *);
  800a12:	8d 40 04             	lea    0x4(%eax),%eax
  800a15:	89 45 14             	mov    %eax,0x14(%ebp)
			base = 16;
  800a18:	bf 10 00 00 00       	mov    $0x10,%edi
			goto number;
  800a1d:	eb 40                	jmp    800a5f <vprintfmt+0x40b>
// Get an unsigned int of various possible sizes from a varargs list,
// depending on the lflag parameter.
static unsigned long long
getuint(va_list *ap, int lflag)
{
	if (lflag >= 2)
  800a1f:	83 f9 01             	cmp    $0x1,%ecx
  800a22:	7e 10                	jle    800a34 <vprintfmt+0x3e0>
		return va_arg(*ap, unsigned long long);
  800a24:	8b 45 14             	mov    0x14(%ebp),%eax
  800a27:	8b 10                	mov    (%eax),%edx
  800a29:	8b 48 04             	mov    0x4(%eax),%ecx
  800a2c:	8d 40 08             	lea    0x8(%eax),%eax
  800a2f:	89 45 14             	mov    %eax,0x14(%ebp)
  800a32:	eb 26                	jmp    800a5a <vprintfmt+0x406>
	else if (lflag)
  800a34:	85 c9                	test   %ecx,%ecx
  800a36:	74 12                	je     800a4a <vprintfmt+0x3f6>
		return va_arg(*ap, unsigned long);
  800a38:	8b 45 14             	mov    0x14(%ebp),%eax
  800a3b:	8b 10                	mov    (%eax),%edx
  800a3d:	b9 00 00 00 00       	mov    $0x0,%ecx
  800a42:	8d 40 04             	lea    0x4(%eax),%eax
  800a45:	89 45 14             	mov    %eax,0x14(%ebp)
  800a48:	eb 10                	jmp    800a5a <vprintfmt+0x406>
	else
		return va_arg(*ap, unsigned int);
  800a4a:	8b 45 14             	mov    0x14(%ebp),%eax
  800a4d:	8b 10                	mov    (%eax),%edx
  800a4f:	b9 00 00 00 00       	mov    $0x0,%ecx
  800a54:	8d 40 04             	lea    0x4(%eax),%eax
  800a57:	89 45 14             	mov    %eax,0x14(%ebp)
			goto number;

		// (unsigned) hexadecimal
		case 'x':
			num = getuint(&ap, lflag);
			base = 16;
  800a5a:	bf 10 00 00 00       	mov    $0x10,%edi
		number:
			printnum(putch, putdat, num, base, width, padc);
  800a5f:	0f be 45 d4          	movsbl -0x2c(%ebp),%eax
  800a63:	89 44 24 10          	mov    %eax,0x10(%esp)
  800a67:	8b 45 e0             	mov    -0x20(%ebp),%eax
  800a6a:	89 44 24 0c          	mov    %eax,0xc(%esp)
  800a6e:	89 7c 24 08          	mov    %edi,0x8(%esp)
  800a72:	89 14 24             	mov    %edx,(%esp)
  800a75:	89 4c 24 04          	mov    %ecx,0x4(%esp)
  800a79:	89 da                	mov    %ebx,%edx
  800a7b:	89 f0                	mov    %esi,%eax
  800a7d:	e8 9e fa ff ff       	call   800520 <printnum>
			break;
  800a82:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  800a85:	e9 ef fb ff ff       	jmp    800679 <vprintfmt+0x25>

		// escaped '%' character
		case '%':
			putch(ch, putdat);
  800a8a:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  800a8e:	89 04 24             	mov    %eax,(%esp)
  800a91:	ff d6                	call   *%esi
		width = -1;
		precision = -1;
		lflag = 0;
		altflag = 0;
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {
  800a93:	8b 7d e4             	mov    -0x1c(%ebp),%edi
			break;

		// escaped '%' character
		case '%':
			putch(ch, putdat);
			break;
  800a96:	e9 de fb ff ff       	jmp    800679 <vprintfmt+0x25>

		// unrecognized escape sequence - just print it literally
		default:
			putch('%', putdat);
  800a9b:	89 5c 24 04          	mov    %ebx,0x4(%esp)
  800a9f:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
  800aa6:	ff d6                	call   *%esi
			for (fmt--; fmt[-1] != '%'; fmt--)
  800aa8:	eb 03                	jmp    800aad <vprintfmt+0x459>
  800aaa:	83 ef 01             	sub    $0x1,%edi
  800aad:	80 7f ff 25          	cmpb   $0x25,-0x1(%edi)
  800ab1:	75 f7                	jne    800aaa <vprintfmt+0x456>
  800ab3:	e9 c1 fb ff ff       	jmp    800679 <vprintfmt+0x25>
				/* do nothing */;
			break;
		}
	}
}
  800ab8:	83 c4 3c             	add    $0x3c,%esp
  800abb:	5b                   	pop    %ebx
  800abc:	5e                   	pop    %esi
  800abd:	5f                   	pop    %edi
  800abe:	5d                   	pop    %ebp
  800abf:	c3                   	ret    

00800ac0 <vsnprintf>:
		*b->buf++ = ch;
}

int
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
  800ac0:	55                   	push   %ebp
  800ac1:	89 e5                	mov    %esp,%ebp
  800ac3:	83 ec 28             	sub    $0x28,%esp
  800ac6:	8b 45 08             	mov    0x8(%ebp),%eax
  800ac9:	8b 55 0c             	mov    0xc(%ebp),%edx
	struct sprintbuf b = {buf, buf+n-1, 0};
  800acc:	89 45 ec             	mov    %eax,-0x14(%ebp)
  800acf:	8d 4c 10 ff          	lea    -0x1(%eax,%edx,1),%ecx
  800ad3:	89 4d f0             	mov    %ecx,-0x10(%ebp)
  800ad6:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

	if (buf == NULL || n < 1)
  800add:	85 c0                	test   %eax,%eax
  800adf:	74 30                	je     800b11 <vsnprintf+0x51>
  800ae1:	85 d2                	test   %edx,%edx
  800ae3:	7e 2c                	jle    800b11 <vsnprintf+0x51>
		return -E_INVAL;

	// print the string to the buffer
	vprintfmt((void*)sprintputch, &b, fmt, ap);
  800ae5:	8b 45 14             	mov    0x14(%ebp),%eax
  800ae8:	89 44 24 0c          	mov    %eax,0xc(%esp)
  800aec:	8b 45 10             	mov    0x10(%ebp),%eax
  800aef:	89 44 24 08          	mov    %eax,0x8(%esp)
  800af3:	8d 45 ec             	lea    -0x14(%ebp),%eax
  800af6:	89 44 24 04          	mov    %eax,0x4(%esp)
  800afa:	c7 04 24 0f 06 80 00 	movl   $0x80060f,(%esp)
  800b01:	e8 4e fb ff ff       	call   800654 <vprintfmt>

	// null terminate the buffer
	*b.buf = '\0';
  800b06:	8b 45 ec             	mov    -0x14(%ebp),%eax
  800b09:	c6 00 00             	movb   $0x0,(%eax)

	return b.cnt;
  800b0c:	8b 45 f4             	mov    -0xc(%ebp),%eax
  800b0f:	eb 05                	jmp    800b16 <vsnprintf+0x56>
vsnprintf(char *buf, int n, const char *fmt, va_list ap)
{
	struct sprintbuf b = {buf, buf+n-1, 0};

	if (buf == NULL || n < 1)
		return -E_INVAL;
  800b11:	b8 fd ff ff ff       	mov    $0xfffffffd,%eax

	// null terminate the buffer
	*b.buf = '\0';

	return b.cnt;
}
  800b16:	c9                   	leave  
  800b17:	c3                   	ret    

00800b18 <snprintf>:

int
snprintf(char *buf, int n, const char *fmt, ...)
{
  800b18:	55                   	push   %ebp
  800b19:	89 e5                	mov    %esp,%ebp
  800b1b:	83 ec 18             	sub    $0x18,%esp
	va_list ap;
	int rc;

	va_start(ap, fmt);
  800b1e:	8d 45 14             	lea    0x14(%ebp),%eax
	rc = vsnprintf(buf, n, fmt, ap);
  800b21:	89 44 24 0c          	mov    %eax,0xc(%esp)
  800b25:	8b 45 10             	mov    0x10(%ebp),%eax
  800b28:	89 44 24 08          	mov    %eax,0x8(%esp)
  800b2c:	8b 45 0c             	mov    0xc(%ebp),%eax
  800b2f:	89 44 24 04          	mov    %eax,0x4(%esp)
  800b33:	8b 45 08             	mov    0x8(%ebp),%eax
  800b36:	89 04 24             	mov    %eax,(%esp)
  800b39:	e8 82 ff ff ff       	call   800ac0 <vsnprintf>
	va_end(ap);

	return rc;
}
  800b3e:	c9                   	leave  
  800b3f:	c3                   	ret    

00800b40 <strlen>:
// Primespipe runs 3x faster this way.
#define ASM 1

int
strlen(const char *s)
{
  800b40:	55                   	push   %ebp
  800b41:	89 e5                	mov    %esp,%ebp
  800b43:	8b 55 08             	mov    0x8(%ebp),%edx
	int n;

	for (n = 0; *s != '\0'; s++)
  800b46:	b8 00 00 00 00       	mov    $0x0,%eax
  800b4b:	eb 03                	jmp    800b50 <strlen+0x10>
		n++;
  800b4d:	83 c0 01             	add    $0x1,%eax
int
strlen(const char *s)
{
	int n;

	for (n = 0; *s != '\0'; s++)
  800b50:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
  800b54:	75 f7                	jne    800b4d <strlen+0xd>
		n++;
	return n;
}
  800b56:	5d                   	pop    %ebp
  800b57:	c3                   	ret    

00800b58 <strnlen>:

int
strnlen(const char *s, size_t size)
{
  800b58:	55                   	push   %ebp
  800b59:	89 e5                	mov    %esp,%ebp
  800b5b:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800b5e:	8b 55 0c             	mov    0xc(%ebp),%edx
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
  800b61:	b8 00 00 00 00       	mov    $0x0,%eax
  800b66:	eb 03                	jmp    800b6b <strnlen+0x13>
		n++;
  800b68:	83 c0 01             	add    $0x1,%eax
int
strnlen(const char *s, size_t size)
{
	int n;

	for (n = 0; size > 0 && *s != '\0'; s++, size--)
  800b6b:	39 d0                	cmp    %edx,%eax
  800b6d:	74 06                	je     800b75 <strnlen+0x1d>
  800b6f:	80 3c 01 00          	cmpb   $0x0,(%ecx,%eax,1)
  800b73:	75 f3                	jne    800b68 <strnlen+0x10>
		n++;
	return n;
}
  800b75:	5d                   	pop    %ebp
  800b76:	c3                   	ret    

00800b77 <strcpy>:

char *
strcpy(char *dst, const char *src)
{
  800b77:	55                   	push   %ebp
  800b78:	89 e5                	mov    %esp,%ebp
  800b7a:	53                   	push   %ebx
  800b7b:	8b 45 08             	mov    0x8(%ebp),%eax
  800b7e:	8b 4d 0c             	mov    0xc(%ebp),%ecx
	char *ret;

	ret = dst;
	while ((*dst++ = *src++) != '\0')
  800b81:	89 c2                	mov    %eax,%edx
  800b83:	83 c2 01             	add    $0x1,%edx
  800b86:	83 c1 01             	add    $0x1,%ecx
  800b89:	0f b6 59 ff          	movzbl -0x1(%ecx),%ebx
  800b8d:	88 5a ff             	mov    %bl,-0x1(%edx)
  800b90:	84 db                	test   %bl,%bl
  800b92:	75 ef                	jne    800b83 <strcpy+0xc>
		/* do nothing */;
	return ret;
}
  800b94:	5b                   	pop    %ebx
  800b95:	5d                   	pop    %ebp
  800b96:	c3                   	ret    

00800b97 <strcat>:

char *
strcat(char *dst, const char *src)
{
  800b97:	55                   	push   %ebp
  800b98:	89 e5                	mov    %esp,%ebp
  800b9a:	53                   	push   %ebx
  800b9b:	83 ec 08             	sub    $0x8,%esp
  800b9e:	8b 5d 08             	mov    0x8(%ebp),%ebx
	int len = strlen(dst);
  800ba1:	89 1c 24             	mov    %ebx,(%esp)
  800ba4:	e8 97 ff ff ff       	call   800b40 <strlen>
	strcpy(dst + len, src);
  800ba9:	8b 55 0c             	mov    0xc(%ebp),%edx
  800bac:	89 54 24 04          	mov    %edx,0x4(%esp)
  800bb0:	01 d8                	add    %ebx,%eax
  800bb2:	89 04 24             	mov    %eax,(%esp)
  800bb5:	e8 bd ff ff ff       	call   800b77 <strcpy>
	return dst;
}
  800bba:	89 d8                	mov    %ebx,%eax
  800bbc:	83 c4 08             	add    $0x8,%esp
  800bbf:	5b                   	pop    %ebx
  800bc0:	5d                   	pop    %ebp
  800bc1:	c3                   	ret    

00800bc2 <strncpy>:

char *
strncpy(char *dst, const char *src, size_t size) {
  800bc2:	55                   	push   %ebp
  800bc3:	89 e5                	mov    %esp,%ebp
  800bc5:	56                   	push   %esi
  800bc6:	53                   	push   %ebx
  800bc7:	8b 75 08             	mov    0x8(%ebp),%esi
  800bca:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800bcd:	89 f3                	mov    %esi,%ebx
  800bcf:	03 5d 10             	add    0x10(%ebp),%ebx
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
  800bd2:	89 f2                	mov    %esi,%edx
  800bd4:	eb 0f                	jmp    800be5 <strncpy+0x23>
		*dst++ = *src;
  800bd6:	83 c2 01             	add    $0x1,%edx
  800bd9:	0f b6 01             	movzbl (%ecx),%eax
  800bdc:	88 42 ff             	mov    %al,-0x1(%edx)
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
  800bdf:	80 39 01             	cmpb   $0x1,(%ecx)
  800be2:	83 d9 ff             	sbb    $0xffffffff,%ecx
strncpy(char *dst, const char *src, size_t size) {
	size_t i;
	char *ret;

	ret = dst;
	for (i = 0; i < size; i++) {
  800be5:	39 da                	cmp    %ebx,%edx
  800be7:	75 ed                	jne    800bd6 <strncpy+0x14>
		// If strlen(src) < size, null-pad 'dst' out to 'size' chars
		if (*src != '\0')
			src++;
	}
	return ret;
}
  800be9:	89 f0                	mov    %esi,%eax
  800beb:	5b                   	pop    %ebx
  800bec:	5e                   	pop    %esi
  800bed:	5d                   	pop    %ebp
  800bee:	c3                   	ret    

00800bef <strlcpy>:

size_t
strlcpy(char *dst, const char *src, size_t size)
{
  800bef:	55                   	push   %ebp
  800bf0:	89 e5                	mov    %esp,%ebp
  800bf2:	56                   	push   %esi
  800bf3:	53                   	push   %ebx
  800bf4:	8b 75 08             	mov    0x8(%ebp),%esi
  800bf7:	8b 55 0c             	mov    0xc(%ebp),%edx
  800bfa:	8b 4d 10             	mov    0x10(%ebp),%ecx
  800bfd:	89 f0                	mov    %esi,%eax
  800bff:	8d 5c 0e ff          	lea    -0x1(%esi,%ecx,1),%ebx
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
  800c03:	85 c9                	test   %ecx,%ecx
  800c05:	75 0b                	jne    800c12 <strlcpy+0x23>
  800c07:	eb 1d                	jmp    800c26 <strlcpy+0x37>
		while (--size > 0 && *src != '\0')
			*dst++ = *src++;
  800c09:	83 c0 01             	add    $0x1,%eax
  800c0c:	83 c2 01             	add    $0x1,%edx
  800c0f:	88 48 ff             	mov    %cl,-0x1(%eax)
{
	char *dst_in;

	dst_in = dst;
	if (size > 0) {
		while (--size > 0 && *src != '\0')
  800c12:	39 d8                	cmp    %ebx,%eax
  800c14:	74 0b                	je     800c21 <strlcpy+0x32>
  800c16:	0f b6 0a             	movzbl (%edx),%ecx
  800c19:	84 c9                	test   %cl,%cl
  800c1b:	75 ec                	jne    800c09 <strlcpy+0x1a>
  800c1d:	89 c2                	mov    %eax,%edx
  800c1f:	eb 02                	jmp    800c23 <strlcpy+0x34>
  800c21:	89 c2                	mov    %eax,%edx
			*dst++ = *src++;
		*dst = '\0';
  800c23:	c6 02 00             	movb   $0x0,(%edx)
	}
	return dst - dst_in;
  800c26:	29 f0                	sub    %esi,%eax
}
  800c28:	5b                   	pop    %ebx
  800c29:	5e                   	pop    %esi
  800c2a:	5d                   	pop    %ebp
  800c2b:	c3                   	ret    

00800c2c <strcmp>:

int
strcmp(const char *p, const char *q)
{
  800c2c:	55                   	push   %ebp
  800c2d:	89 e5                	mov    %esp,%ebp
  800c2f:	8b 4d 08             	mov    0x8(%ebp),%ecx
  800c32:	8b 55 0c             	mov    0xc(%ebp),%edx
	while (*p && *p == *q)
  800c35:	eb 06                	jmp    800c3d <strcmp+0x11>
		p++, q++;
  800c37:	83 c1 01             	add    $0x1,%ecx
  800c3a:	83 c2 01             	add    $0x1,%edx
}

int
strcmp(const char *p, const char *q)
{
	while (*p && *p == *q)
  800c3d:	0f b6 01             	movzbl (%ecx),%eax
  800c40:	84 c0                	test   %al,%al
  800c42:	74 04                	je     800c48 <strcmp+0x1c>
  800c44:	3a 02                	cmp    (%edx),%al
  800c46:	74 ef                	je     800c37 <strcmp+0xb>
		p++, q++;
	return (int) ((unsigned char) *p - (unsigned char) *q);
  800c48:	0f b6 c0             	movzbl %al,%eax
  800c4b:	0f b6 12             	movzbl (%edx),%edx
  800c4e:	29 d0                	sub    %edx,%eax
}
  800c50:	5d                   	pop    %ebp
  800c51:	c3                   	ret    

00800c52 <strncmp>:

int
strncmp(const char *p, const char *q, size_t n)
{
  800c52:	55                   	push   %ebp
  800c53:	89 e5                	mov    %esp,%ebp
  800c55:	53                   	push   %ebx
  800c56:	8b 45 08             	mov    0x8(%ebp),%eax
  800c59:	8b 55 0c             	mov    0xc(%ebp),%edx
  800c5c:	89 c3                	mov    %eax,%ebx
  800c5e:	03 5d 10             	add    0x10(%ebp),%ebx
	while (n > 0 && *p && *p == *q)
  800c61:	eb 06                	jmp    800c69 <strncmp+0x17>
		n--, p++, q++;
  800c63:	83 c0 01             	add    $0x1,%eax
  800c66:	83 c2 01             	add    $0x1,%edx
}

int
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
  800c69:	39 d8                	cmp    %ebx,%eax
  800c6b:	74 15                	je     800c82 <strncmp+0x30>
  800c6d:	0f b6 08             	movzbl (%eax),%ecx
  800c70:	84 c9                	test   %cl,%cl
  800c72:	74 04                	je     800c78 <strncmp+0x26>
  800c74:	3a 0a                	cmp    (%edx),%cl
  800c76:	74 eb                	je     800c63 <strncmp+0x11>
		n--, p++, q++;
	if (n == 0)
		return 0;
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
  800c78:	0f b6 00             	movzbl (%eax),%eax
  800c7b:	0f b6 12             	movzbl (%edx),%edx
  800c7e:	29 d0                	sub    %edx,%eax
  800c80:	eb 05                	jmp    800c87 <strncmp+0x35>
strncmp(const char *p, const char *q, size_t n)
{
	while (n > 0 && *p && *p == *q)
		n--, p++, q++;
	if (n == 0)
		return 0;
  800c82:	b8 00 00 00 00       	mov    $0x0,%eax
	else
		return (int) ((unsigned char) *p - (unsigned char) *q);
}
  800c87:	5b                   	pop    %ebx
  800c88:	5d                   	pop    %ebp
  800c89:	c3                   	ret    

00800c8a <strchr>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
  800c8a:	55                   	push   %ebp
  800c8b:	89 e5                	mov    %esp,%ebp
  800c8d:	8b 45 08             	mov    0x8(%ebp),%eax
  800c90:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
  800c94:	eb 07                	jmp    800c9d <strchr+0x13>
		if (*s == c)
  800c96:	38 ca                	cmp    %cl,%dl
  800c98:	74 0f                	je     800ca9 <strchr+0x1f>
// Return a pointer to the first occurrence of 'c' in 's',
// or a null pointer if the string has no 'c'.
char *
strchr(const char *s, char c)
{
	for (; *s; s++)
  800c9a:	83 c0 01             	add    $0x1,%eax
  800c9d:	0f b6 10             	movzbl (%eax),%edx
  800ca0:	84 d2                	test   %dl,%dl
  800ca2:	75 f2                	jne    800c96 <strchr+0xc>
		if (*s == c)
			return (char *) s;
	return 0;
  800ca4:	b8 00 00 00 00       	mov    $0x0,%eax
}
  800ca9:	5d                   	pop    %ebp
  800caa:	c3                   	ret    

00800cab <strfind>:

// Return a pointer to the first occurrence of 'c' in 's',
// or a pointer to the string-ending null character if the string has no 'c'.
char *
strfind(const char *s, char c)
{
  800cab:	55                   	push   %ebp
  800cac:	89 e5                	mov    %esp,%ebp
  800cae:	8b 45 08             	mov    0x8(%ebp),%eax
  800cb1:	0f b6 4d 0c          	movzbl 0xc(%ebp),%ecx
	for (; *s; s++)
  800cb5:	eb 07                	jmp    800cbe <strfind+0x13>
		if (*s == c)
  800cb7:	38 ca                	cmp    %cl,%dl
  800cb9:	74 0a                	je     800cc5 <strfind+0x1a>
// Return a pointer to the first occurrence of 'c' in 's',
// or a pointer to the string-ending null character if the string has no 'c'.
char *
strfind(const char *s, char c)
{
	for (; *s; s++)
  800cbb:	83 c0 01             	add    $0x1,%eax
  800cbe:	0f b6 10             	movzbl (%eax),%edx
  800cc1:	84 d2                	test   %dl,%dl
  800cc3:	75 f2                	jne    800cb7 <strfind+0xc>
		if (*s == c)
			break;
	return (char *) s;
}
  800cc5:	5d                   	pop    %ebp
  800cc6:	c3                   	ret    

00800cc7 <memset>:

#if ASM
void *
memset(void *v, int c, size_t n)
{
  800cc7:	55                   	push   %ebp
  800cc8:	89 e5                	mov    %esp,%ebp
  800cca:	57                   	push   %edi
  800ccb:	56                   	push   %esi
  800ccc:	53                   	push   %ebx
  800ccd:	8b 7d 08             	mov    0x8(%ebp),%edi
  800cd0:	8b 4d 10             	mov    0x10(%ebp),%ecx
	char *p;

	if (n == 0)
  800cd3:	85 c9                	test   %ecx,%ecx
  800cd5:	74 36                	je     800d0d <memset+0x46>
		return v;
	if ((int)v%4 == 0 && n%4 == 0) {
  800cd7:	f7 c7 03 00 00 00    	test   $0x3,%edi
  800cdd:	75 28                	jne    800d07 <memset+0x40>
  800cdf:	f6 c1 03             	test   $0x3,%cl
  800ce2:	75 23                	jne    800d07 <memset+0x40>
		c &= 0xFF;
  800ce4:	0f b6 55 0c          	movzbl 0xc(%ebp),%edx
		c = (c<<24)|(c<<16)|(c<<8)|c;
  800ce8:	89 d3                	mov    %edx,%ebx
  800cea:	c1 e3 08             	shl    $0x8,%ebx
  800ced:	89 d6                	mov    %edx,%esi
  800cef:	c1 e6 18             	shl    $0x18,%esi
  800cf2:	89 d0                	mov    %edx,%eax
  800cf4:	c1 e0 10             	shl    $0x10,%eax
  800cf7:	09 f0                	or     %esi,%eax
  800cf9:	09 c2                	or     %eax,%edx
  800cfb:	89 d0                	mov    %edx,%eax
  800cfd:	09 d8                	or     %ebx,%eax
		asm volatile("cld; rep stosl\n"
			:: "D" (v), "a" (c), "c" (n/4)
  800cff:	c1 e9 02             	shr    $0x2,%ecx
	if (n == 0)
		return v;
	if ((int)v%4 == 0 && n%4 == 0) {
		c &= 0xFF;
		c = (c<<24)|(c<<16)|(c<<8)|c;
		asm volatile("cld; rep stosl\n"
  800d02:	fc                   	cld    
  800d03:	f3 ab                	rep stos %eax,%es:(%edi)
  800d05:	eb 06                	jmp    800d0d <memset+0x46>
			:: "D" (v), "a" (c), "c" (n/4)
			: "cc", "memory");
	} else
		asm volatile("cld; rep stosb\n"
  800d07:	8b 45 0c             	mov    0xc(%ebp),%eax
  800d0a:	fc                   	cld    
  800d0b:	f3 aa                	rep stos %al,%es:(%edi)
			:: "D" (v), "a" (c), "c" (n)
			: "cc", "memory");
	return v;
}
  800d0d:	89 f8                	mov    %edi,%eax
  800d0f:	5b                   	pop    %ebx
  800d10:	5e                   	pop    %esi
  800d11:	5f                   	pop    %edi
  800d12:	5d                   	pop    %ebp
  800d13:	c3                   	ret    

00800d14 <memmove>:

void *
memmove(void *dst, const void *src, size_t n)
{
  800d14:	55                   	push   %ebp
  800d15:	89 e5                	mov    %esp,%ebp
  800d17:	57                   	push   %edi
  800d18:	56                   	push   %esi
  800d19:	8b 45 08             	mov    0x8(%ebp),%eax
  800d1c:	8b 75 0c             	mov    0xc(%ebp),%esi
  800d1f:	8b 4d 10             	mov    0x10(%ebp),%ecx
	const char *s;
	char *d;

	s = src;
	d = dst;
	if (s < d && s + n > d) {
  800d22:	39 c6                	cmp    %eax,%esi
  800d24:	73 35                	jae    800d5b <memmove+0x47>
  800d26:	8d 14 0e             	lea    (%esi,%ecx,1),%edx
  800d29:	39 d0                	cmp    %edx,%eax
  800d2b:	73 2e                	jae    800d5b <memmove+0x47>
		s += n;
		d += n;
  800d2d:	8d 3c 08             	lea    (%eax,%ecx,1),%edi
  800d30:	89 d6                	mov    %edx,%esi
  800d32:	09 fe                	or     %edi,%esi
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
  800d34:	f7 c6 03 00 00 00    	test   $0x3,%esi
  800d3a:	75 13                	jne    800d4f <memmove+0x3b>
  800d3c:	f6 c1 03             	test   $0x3,%cl
  800d3f:	75 0e                	jne    800d4f <memmove+0x3b>
			asm volatile("std; rep movsl\n"
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
  800d41:	83 ef 04             	sub    $0x4,%edi
  800d44:	8d 72 fc             	lea    -0x4(%edx),%esi
  800d47:	c1 e9 02             	shr    $0x2,%ecx
	d = dst;
	if (s < d && s + n > d) {
		s += n;
		d += n;
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("std; rep movsl\n"
  800d4a:	fd                   	std    
  800d4b:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  800d4d:	eb 09                	jmp    800d58 <memmove+0x44>
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
		else
			asm volatile("std; rep movsb\n"
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
  800d4f:	83 ef 01             	sub    $0x1,%edi
  800d52:	8d 72 ff             	lea    -0x1(%edx),%esi
		d += n;
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("std; rep movsl\n"
				:: "D" (d-4), "S" (s-4), "c" (n/4) : "cc", "memory");
		else
			asm volatile("std; rep movsb\n"
  800d55:	fd                   	std    
  800d56:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
		// Some versions of GCC rely on DF being clear
		asm volatile("cld" ::: "cc");
  800d58:	fc                   	cld    
  800d59:	eb 1d                	jmp    800d78 <memmove+0x64>
  800d5b:	89 f2                	mov    %esi,%edx
  800d5d:	09 c2                	or     %eax,%edx
	} else {
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
  800d5f:	f6 c2 03             	test   $0x3,%dl
  800d62:	75 0f                	jne    800d73 <memmove+0x5f>
  800d64:	f6 c1 03             	test   $0x3,%cl
  800d67:	75 0a                	jne    800d73 <memmove+0x5f>
			asm volatile("cld; rep movsl\n"
				:: "D" (d), "S" (s), "c" (n/4) : "cc", "memory");
  800d69:	c1 e9 02             	shr    $0x2,%ecx
				:: "D" (d-1), "S" (s-1), "c" (n) : "cc", "memory");
		// Some versions of GCC rely on DF being clear
		asm volatile("cld" ::: "cc");
	} else {
		if ((int)s%4 == 0 && (int)d%4 == 0 && n%4 == 0)
			asm volatile("cld; rep movsl\n"
  800d6c:	89 c7                	mov    %eax,%edi
  800d6e:	fc                   	cld    
  800d6f:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  800d71:	eb 05                	jmp    800d78 <memmove+0x64>
				:: "D" (d), "S" (s), "c" (n/4) : "cc", "memory");
		else
			asm volatile("cld; rep movsb\n"
  800d73:	89 c7                	mov    %eax,%edi
  800d75:	fc                   	cld    
  800d76:	f3 a4                	rep movsb %ds:(%esi),%es:(%edi)
				:: "D" (d), "S" (s), "c" (n) : "cc", "memory");
	}
	return dst;
}
  800d78:	5e                   	pop    %esi
  800d79:	5f                   	pop    %edi
  800d7a:	5d                   	pop    %ebp
  800d7b:	c3                   	ret    

00800d7c <memcpy>:
}
#endif

void *
memcpy(void *dst, const void *src, size_t n)
{
  800d7c:	55                   	push   %ebp
  800d7d:	89 e5                	mov    %esp,%ebp
  800d7f:	83 ec 0c             	sub    $0xc,%esp
	return memmove(dst, src, n);
  800d82:	8b 45 10             	mov    0x10(%ebp),%eax
  800d85:	89 44 24 08          	mov    %eax,0x8(%esp)
  800d89:	8b 45 0c             	mov    0xc(%ebp),%eax
  800d8c:	89 44 24 04          	mov    %eax,0x4(%esp)
  800d90:	8b 45 08             	mov    0x8(%ebp),%eax
  800d93:	89 04 24             	mov    %eax,(%esp)
  800d96:	e8 79 ff ff ff       	call   800d14 <memmove>
}
  800d9b:	c9                   	leave  
  800d9c:	c3                   	ret    

00800d9d <memcmp>:

int
memcmp(const void *v1, const void *v2, size_t n)
{
  800d9d:	55                   	push   %ebp
  800d9e:	89 e5                	mov    %esp,%ebp
  800da0:	56                   	push   %esi
  800da1:	53                   	push   %ebx
  800da2:	8b 55 08             	mov    0x8(%ebp),%edx
  800da5:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  800da8:	89 d6                	mov    %edx,%esi
  800daa:	03 75 10             	add    0x10(%ebp),%esi
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
  800dad:	eb 1a                	jmp    800dc9 <memcmp+0x2c>
		if (*s1 != *s2)
  800daf:	0f b6 02             	movzbl (%edx),%eax
  800db2:	0f b6 19             	movzbl (%ecx),%ebx
  800db5:	38 d8                	cmp    %bl,%al
  800db7:	74 0a                	je     800dc3 <memcmp+0x26>
			return (int) *s1 - (int) *s2;
  800db9:	0f b6 c0             	movzbl %al,%eax
  800dbc:	0f b6 db             	movzbl %bl,%ebx
  800dbf:	29 d8                	sub    %ebx,%eax
  800dc1:	eb 0f                	jmp    800dd2 <memcmp+0x35>
		s1++, s2++;
  800dc3:	83 c2 01             	add    $0x1,%edx
  800dc6:	83 c1 01             	add    $0x1,%ecx
memcmp(const void *v1, const void *v2, size_t n)
{
	const uint8_t *s1 = (const uint8_t *) v1;
	const uint8_t *s2 = (const uint8_t *) v2;

	while (n-- > 0) {
  800dc9:	39 f2                	cmp    %esi,%edx
  800dcb:	75 e2                	jne    800daf <memcmp+0x12>
		if (*s1 != *s2)
			return (int) *s1 - (int) *s2;
		s1++, s2++;
	}

	return 0;
  800dcd:	b8 00 00 00 00       	mov    $0x0,%eax
}
  800dd2:	5b                   	pop    %ebx
  800dd3:	5e                   	pop    %esi
  800dd4:	5d                   	pop    %ebp
  800dd5:	c3                   	ret    

00800dd6 <memfind>:

void *
memfind(const void *s, int c, size_t n)
{
  800dd6:	55                   	push   %ebp
  800dd7:	89 e5                	mov    %esp,%ebp
  800dd9:	8b 45 08             	mov    0x8(%ebp),%eax
  800ddc:	8b 4d 0c             	mov    0xc(%ebp),%ecx
	const void *ends = (const char *) s + n;
  800ddf:	89 c2                	mov    %eax,%edx
  800de1:	03 55 10             	add    0x10(%ebp),%edx
	for (; s < ends; s++)
  800de4:	eb 07                	jmp    800ded <memfind+0x17>
		if (*(const unsigned char *) s == (unsigned char) c)
  800de6:	38 08                	cmp    %cl,(%eax)
  800de8:	74 07                	je     800df1 <memfind+0x1b>

void *
memfind(const void *s, int c, size_t n)
{
	const void *ends = (const char *) s + n;
	for (; s < ends; s++)
  800dea:	83 c0 01             	add    $0x1,%eax
  800ded:	39 d0                	cmp    %edx,%eax
  800def:	72 f5                	jb     800de6 <memfind+0x10>
		if (*(const unsigned char *) s == (unsigned char) c)
			break;
	return (void *) s;
}
  800df1:	5d                   	pop    %ebp
  800df2:	c3                   	ret    

00800df3 <strtol>:

long
strtol(const char *s, char **endptr, int base)
{
  800df3:	55                   	push   %ebp
  800df4:	89 e5                	mov    %esp,%ebp
  800df6:	57                   	push   %edi
  800df7:	56                   	push   %esi
  800df8:	53                   	push   %ebx
  800df9:	8b 55 08             	mov    0x8(%ebp),%edx
  800dfc:	8b 45 10             	mov    0x10(%ebp),%eax
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
  800dff:	eb 03                	jmp    800e04 <strtol+0x11>
		s++;
  800e01:	83 c2 01             	add    $0x1,%edx
{
	int neg = 0;
	long val = 0;

	// gobble initial whitespace
	while (*s == ' ' || *s == '\t')
  800e04:	0f b6 0a             	movzbl (%edx),%ecx
  800e07:	80 f9 09             	cmp    $0x9,%cl
  800e0a:	74 f5                	je     800e01 <strtol+0xe>
  800e0c:	80 f9 20             	cmp    $0x20,%cl
  800e0f:	74 f0                	je     800e01 <strtol+0xe>
		s++;

	// plus/minus sign
	if (*s == '+')
  800e11:	80 f9 2b             	cmp    $0x2b,%cl
  800e14:	75 0a                	jne    800e20 <strtol+0x2d>
		s++;
  800e16:	83 c2 01             	add    $0x1,%edx
}

long
strtol(const char *s, char **endptr, int base)
{
	int neg = 0;
  800e19:	bf 00 00 00 00       	mov    $0x0,%edi
  800e1e:	eb 11                	jmp    800e31 <strtol+0x3e>
  800e20:	bf 00 00 00 00       	mov    $0x0,%edi
		s++;

	// plus/minus sign
	if (*s == '+')
		s++;
	else if (*s == '-')
  800e25:	80 f9 2d             	cmp    $0x2d,%cl
  800e28:	75 07                	jne    800e31 <strtol+0x3e>
		s++, neg = 1;
  800e2a:	8d 52 01             	lea    0x1(%edx),%edx
  800e2d:	66 bf 01 00          	mov    $0x1,%di

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
  800e31:	a9 ef ff ff ff       	test   $0xffffffef,%eax
  800e36:	75 15                	jne    800e4d <strtol+0x5a>
  800e38:	80 3a 30             	cmpb   $0x30,(%edx)
  800e3b:	75 10                	jne    800e4d <strtol+0x5a>
  800e3d:	80 7a 01 78          	cmpb   $0x78,0x1(%edx)
  800e41:	75 0a                	jne    800e4d <strtol+0x5a>
		s += 2, base = 16;
  800e43:	83 c2 02             	add    $0x2,%edx
  800e46:	b8 10 00 00 00       	mov    $0x10,%eax
  800e4b:	eb 10                	jmp    800e5d <strtol+0x6a>
	else if (base == 0 && s[0] == '0')
  800e4d:	85 c0                	test   %eax,%eax
  800e4f:	75 0c                	jne    800e5d <strtol+0x6a>
		s++, base = 8;
	else if (base == 0)
		base = 10;
  800e51:	b0 0a                	mov    $0xa,%al
		s++, neg = 1;

	// hex or octal base prefix
	if ((base == 0 || base == 16) && (s[0] == '0' && s[1] == 'x'))
		s += 2, base = 16;
	else if (base == 0 && s[0] == '0')
  800e53:	80 3a 30             	cmpb   $0x30,(%edx)
  800e56:	75 05                	jne    800e5d <strtol+0x6a>
		s++, base = 8;
  800e58:	83 c2 01             	add    $0x1,%edx
  800e5b:	b0 08                	mov    $0x8,%al
	else if (base == 0)
		base = 10;
  800e5d:	bb 00 00 00 00       	mov    $0x0,%ebx
  800e62:	89 45 10             	mov    %eax,0x10(%ebp)

	// digits
	while (1) {
		int dig;

		if (*s >= '0' && *s <= '9')
  800e65:	0f b6 0a             	movzbl (%edx),%ecx
  800e68:	8d 71 d0             	lea    -0x30(%ecx),%esi
  800e6b:	89 f0                	mov    %esi,%eax
  800e6d:	3c 09                	cmp    $0x9,%al
  800e6f:	77 08                	ja     800e79 <strtol+0x86>
			dig = *s - '0';
  800e71:	0f be c9             	movsbl %cl,%ecx
  800e74:	83 e9 30             	sub    $0x30,%ecx
  800e77:	eb 20                	jmp    800e99 <strtol+0xa6>
		else if (*s >= 'a' && *s <= 'z')
  800e79:	8d 71 9f             	lea    -0x61(%ecx),%esi
  800e7c:	89 f0                	mov    %esi,%eax
  800e7e:	3c 19                	cmp    $0x19,%al
  800e80:	77 08                	ja     800e8a <strtol+0x97>
			dig = *s - 'a' + 10;
  800e82:	0f be c9             	movsbl %cl,%ecx
  800e85:	83 e9 57             	sub    $0x57,%ecx
  800e88:	eb 0f                	jmp    800e99 <strtol+0xa6>
		else if (*s >= 'A' && *s <= 'Z')
  800e8a:	8d 71 bf             	lea    -0x41(%ecx),%esi
  800e8d:	89 f0                	mov    %esi,%eax
  800e8f:	3c 19                	cmp    $0x19,%al
  800e91:	77 16                	ja     800ea9 <strtol+0xb6>
			dig = *s - 'A' + 10;
  800e93:	0f be c9             	movsbl %cl,%ecx
  800e96:	83 e9 37             	sub    $0x37,%ecx
		else
			break;
		if (dig >= base)
  800e99:	3b 4d 10             	cmp    0x10(%ebp),%ecx
  800e9c:	7d 0f                	jge    800ead <strtol+0xba>
			break;
		s++, val = (val * base) + dig;
  800e9e:	83 c2 01             	add    $0x1,%edx
  800ea1:	0f af 5d 10          	imul   0x10(%ebp),%ebx
  800ea5:	01 cb                	add    %ecx,%ebx
		// we don't properly detect overflow!
	}
  800ea7:	eb bc                	jmp    800e65 <strtol+0x72>
  800ea9:	89 d8                	mov    %ebx,%eax
  800eab:	eb 02                	jmp    800eaf <strtol+0xbc>
  800ead:	89 d8                	mov    %ebx,%eax

	if (endptr)
  800eaf:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
  800eb3:	74 05                	je     800eba <strtol+0xc7>
		*endptr = (char *) s;
  800eb5:	8b 75 0c             	mov    0xc(%ebp),%esi
  800eb8:	89 16                	mov    %edx,(%esi)
	return (neg ? -val : val);
  800eba:	f7 d8                	neg    %eax
  800ebc:	85 ff                	test   %edi,%edi
  800ebe:	0f 44 c3             	cmove  %ebx,%eax
}
  800ec1:	5b                   	pop    %ebx
  800ec2:	5e                   	pop    %esi
  800ec3:	5f                   	pop    %edi
  800ec4:	5d                   	pop    %ebp
  800ec5:	c3                   	ret    
  800ec6:	66 90                	xchg   %ax,%ax
  800ec8:	66 90                	xchg   %ax,%ax
  800eca:	66 90                	xchg   %ax,%ax
  800ecc:	66 90                	xchg   %ax,%ax
  800ece:	66 90                	xchg   %ax,%ax

00800ed0 <__udivdi3>:
  800ed0:	55                   	push   %ebp
  800ed1:	57                   	push   %edi
  800ed2:	56                   	push   %esi
  800ed3:	83 ec 0c             	sub    $0xc,%esp
  800ed6:	8b 44 24 28          	mov    0x28(%esp),%eax
  800eda:	8b 7c 24 1c          	mov    0x1c(%esp),%edi
  800ede:	8b 6c 24 20          	mov    0x20(%esp),%ebp
  800ee2:	8b 4c 24 24          	mov    0x24(%esp),%ecx
  800ee6:	85 c0                	test   %eax,%eax
  800ee8:	89 7c 24 04          	mov    %edi,0x4(%esp)
  800eec:	89 ea                	mov    %ebp,%edx
  800eee:	89 0c 24             	mov    %ecx,(%esp)
  800ef1:	75 2d                	jne    800f20 <__udivdi3+0x50>
  800ef3:	39 e9                	cmp    %ebp,%ecx
  800ef5:	77 61                	ja     800f58 <__udivdi3+0x88>
  800ef7:	85 c9                	test   %ecx,%ecx
  800ef9:	89 ce                	mov    %ecx,%esi
  800efb:	75 0b                	jne    800f08 <__udivdi3+0x38>
  800efd:	b8 01 00 00 00       	mov    $0x1,%eax
  800f02:	31 d2                	xor    %edx,%edx
  800f04:	f7 f1                	div    %ecx
  800f06:	89 c6                	mov    %eax,%esi
  800f08:	31 d2                	xor    %edx,%edx
  800f0a:	89 e8                	mov    %ebp,%eax
  800f0c:	f7 f6                	div    %esi
  800f0e:	89 c5                	mov    %eax,%ebp
  800f10:	89 f8                	mov    %edi,%eax
  800f12:	f7 f6                	div    %esi
  800f14:	89 ea                	mov    %ebp,%edx
  800f16:	83 c4 0c             	add    $0xc,%esp
  800f19:	5e                   	pop    %esi
  800f1a:	5f                   	pop    %edi
  800f1b:	5d                   	pop    %ebp
  800f1c:	c3                   	ret    
  800f1d:	8d 76 00             	lea    0x0(%esi),%esi
  800f20:	39 e8                	cmp    %ebp,%eax
  800f22:	77 24                	ja     800f48 <__udivdi3+0x78>
  800f24:	0f bd e8             	bsr    %eax,%ebp
  800f27:	83 f5 1f             	xor    $0x1f,%ebp
  800f2a:	75 3c                	jne    800f68 <__udivdi3+0x98>
  800f2c:	8b 74 24 04          	mov    0x4(%esp),%esi
  800f30:	39 34 24             	cmp    %esi,(%esp)
  800f33:	0f 86 9f 00 00 00    	jbe    800fd8 <__udivdi3+0x108>
  800f39:	39 d0                	cmp    %edx,%eax
  800f3b:	0f 82 97 00 00 00    	jb     800fd8 <__udivdi3+0x108>
  800f41:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  800f48:	31 d2                	xor    %edx,%edx
  800f4a:	31 c0                	xor    %eax,%eax
  800f4c:	83 c4 0c             	add    $0xc,%esp
  800f4f:	5e                   	pop    %esi
  800f50:	5f                   	pop    %edi
  800f51:	5d                   	pop    %ebp
  800f52:	c3                   	ret    
  800f53:	90                   	nop
  800f54:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  800f58:	89 f8                	mov    %edi,%eax
  800f5a:	f7 f1                	div    %ecx
  800f5c:	31 d2                	xor    %edx,%edx
  800f5e:	83 c4 0c             	add    $0xc,%esp
  800f61:	5e                   	pop    %esi
  800f62:	5f                   	pop    %edi
  800f63:	5d                   	pop    %ebp
  800f64:	c3                   	ret    
  800f65:	8d 76 00             	lea    0x0(%esi),%esi
  800f68:	89 e9                	mov    %ebp,%ecx
  800f6a:	8b 3c 24             	mov    (%esp),%edi
  800f6d:	d3 e0                	shl    %cl,%eax
  800f6f:	89 c6                	mov    %eax,%esi
  800f71:	b8 20 00 00 00       	mov    $0x20,%eax
  800f76:	29 e8                	sub    %ebp,%eax
  800f78:	89 c1                	mov    %eax,%ecx
  800f7a:	d3 ef                	shr    %cl,%edi
  800f7c:	89 e9                	mov    %ebp,%ecx
  800f7e:	89 7c 24 08          	mov    %edi,0x8(%esp)
  800f82:	8b 3c 24             	mov    (%esp),%edi
  800f85:	09 74 24 08          	or     %esi,0x8(%esp)
  800f89:	89 d6                	mov    %edx,%esi
  800f8b:	d3 e7                	shl    %cl,%edi
  800f8d:	89 c1                	mov    %eax,%ecx
  800f8f:	89 3c 24             	mov    %edi,(%esp)
  800f92:	8b 7c 24 04          	mov    0x4(%esp),%edi
  800f96:	d3 ee                	shr    %cl,%esi
  800f98:	89 e9                	mov    %ebp,%ecx
  800f9a:	d3 e2                	shl    %cl,%edx
  800f9c:	89 c1                	mov    %eax,%ecx
  800f9e:	d3 ef                	shr    %cl,%edi
  800fa0:	09 d7                	or     %edx,%edi
  800fa2:	89 f2                	mov    %esi,%edx
  800fa4:	89 f8                	mov    %edi,%eax
  800fa6:	f7 74 24 08          	divl   0x8(%esp)
  800faa:	89 d6                	mov    %edx,%esi
  800fac:	89 c7                	mov    %eax,%edi
  800fae:	f7 24 24             	mull   (%esp)
  800fb1:	39 d6                	cmp    %edx,%esi
  800fb3:	89 14 24             	mov    %edx,(%esp)
  800fb6:	72 30                	jb     800fe8 <__udivdi3+0x118>
  800fb8:	8b 54 24 04          	mov    0x4(%esp),%edx
  800fbc:	89 e9                	mov    %ebp,%ecx
  800fbe:	d3 e2                	shl    %cl,%edx
  800fc0:	39 c2                	cmp    %eax,%edx
  800fc2:	73 05                	jae    800fc9 <__udivdi3+0xf9>
  800fc4:	3b 34 24             	cmp    (%esp),%esi
  800fc7:	74 1f                	je     800fe8 <__udivdi3+0x118>
  800fc9:	89 f8                	mov    %edi,%eax
  800fcb:	31 d2                	xor    %edx,%edx
  800fcd:	e9 7a ff ff ff       	jmp    800f4c <__udivdi3+0x7c>
  800fd2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  800fd8:	31 d2                	xor    %edx,%edx
  800fda:	b8 01 00 00 00       	mov    $0x1,%eax
  800fdf:	e9 68 ff ff ff       	jmp    800f4c <__udivdi3+0x7c>
  800fe4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  800fe8:	8d 47 ff             	lea    -0x1(%edi),%eax
  800feb:	31 d2                	xor    %edx,%edx
  800fed:	83 c4 0c             	add    $0xc,%esp
  800ff0:	5e                   	pop    %esi
  800ff1:	5f                   	pop    %edi
  800ff2:	5d                   	pop    %ebp
  800ff3:	c3                   	ret    
  800ff4:	66 90                	xchg   %ax,%ax
  800ff6:	66 90                	xchg   %ax,%ax
  800ff8:	66 90                	xchg   %ax,%ax
  800ffa:	66 90                	xchg   %ax,%ax
  800ffc:	66 90                	xchg   %ax,%ax
  800ffe:	66 90                	xchg   %ax,%ax

00801000 <__umoddi3>:
  801000:	55                   	push   %ebp
  801001:	57                   	push   %edi
  801002:	56                   	push   %esi
  801003:	83 ec 14             	sub    $0x14,%esp
  801006:	8b 44 24 28          	mov    0x28(%esp),%eax
  80100a:	8b 4c 24 24          	mov    0x24(%esp),%ecx
  80100e:	8b 74 24 2c          	mov    0x2c(%esp),%esi
  801012:	89 c7                	mov    %eax,%edi
  801014:	89 44 24 04          	mov    %eax,0x4(%esp)
  801018:	8b 44 24 30          	mov    0x30(%esp),%eax
  80101c:	89 4c 24 10          	mov    %ecx,0x10(%esp)
  801020:	89 34 24             	mov    %esi,(%esp)
  801023:	89 4c 24 08          	mov    %ecx,0x8(%esp)
  801027:	85 c0                	test   %eax,%eax
  801029:	89 c2                	mov    %eax,%edx
  80102b:	89 7c 24 0c          	mov    %edi,0xc(%esp)
  80102f:	75 17                	jne    801048 <__umoddi3+0x48>
  801031:	39 fe                	cmp    %edi,%esi
  801033:	76 4b                	jbe    801080 <__umoddi3+0x80>
  801035:	89 c8                	mov    %ecx,%eax
  801037:	89 fa                	mov    %edi,%edx
  801039:	f7 f6                	div    %esi
  80103b:	89 d0                	mov    %edx,%eax
  80103d:	31 d2                	xor    %edx,%edx
  80103f:	83 c4 14             	add    $0x14,%esp
  801042:	5e                   	pop    %esi
  801043:	5f                   	pop    %edi
  801044:	5d                   	pop    %ebp
  801045:	c3                   	ret    
  801046:	66 90                	xchg   %ax,%ax
  801048:	39 f8                	cmp    %edi,%eax
  80104a:	77 54                	ja     8010a0 <__umoddi3+0xa0>
  80104c:	0f bd e8             	bsr    %eax,%ebp
  80104f:	83 f5 1f             	xor    $0x1f,%ebp
  801052:	75 5c                	jne    8010b0 <__umoddi3+0xb0>
  801054:	8b 7c 24 08          	mov    0x8(%esp),%edi
  801058:	39 3c 24             	cmp    %edi,(%esp)
  80105b:	0f 87 e7 00 00 00    	ja     801148 <__umoddi3+0x148>
  801061:	8b 7c 24 04          	mov    0x4(%esp),%edi
  801065:	29 f1                	sub    %esi,%ecx
  801067:	19 c7                	sbb    %eax,%edi
  801069:	89 4c 24 08          	mov    %ecx,0x8(%esp)
  80106d:	89 7c 24 0c          	mov    %edi,0xc(%esp)
  801071:	8b 44 24 08          	mov    0x8(%esp),%eax
  801075:	8b 54 24 0c          	mov    0xc(%esp),%edx
  801079:	83 c4 14             	add    $0x14,%esp
  80107c:	5e                   	pop    %esi
  80107d:	5f                   	pop    %edi
  80107e:	5d                   	pop    %ebp
  80107f:	c3                   	ret    
  801080:	85 f6                	test   %esi,%esi
  801082:	89 f5                	mov    %esi,%ebp
  801084:	75 0b                	jne    801091 <__umoddi3+0x91>
  801086:	b8 01 00 00 00       	mov    $0x1,%eax
  80108b:	31 d2                	xor    %edx,%edx
  80108d:	f7 f6                	div    %esi
  80108f:	89 c5                	mov    %eax,%ebp
  801091:	8b 44 24 04          	mov    0x4(%esp),%eax
  801095:	31 d2                	xor    %edx,%edx
  801097:	f7 f5                	div    %ebp
  801099:	89 c8                	mov    %ecx,%eax
  80109b:	f7 f5                	div    %ebp
  80109d:	eb 9c                	jmp    80103b <__umoddi3+0x3b>
  80109f:	90                   	nop
  8010a0:	89 c8                	mov    %ecx,%eax
  8010a2:	89 fa                	mov    %edi,%edx
  8010a4:	83 c4 14             	add    $0x14,%esp
  8010a7:	5e                   	pop    %esi
  8010a8:	5f                   	pop    %edi
  8010a9:	5d                   	pop    %ebp
  8010aa:	c3                   	ret    
  8010ab:	90                   	nop
  8010ac:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  8010b0:	8b 04 24             	mov    (%esp),%eax
  8010b3:	be 20 00 00 00       	mov    $0x20,%esi
  8010b8:	89 e9                	mov    %ebp,%ecx
  8010ba:	29 ee                	sub    %ebp,%esi
  8010bc:	d3 e2                	shl    %cl,%edx
  8010be:	89 f1                	mov    %esi,%ecx
  8010c0:	d3 e8                	shr    %cl,%eax
  8010c2:	89 e9                	mov    %ebp,%ecx
  8010c4:	89 44 24 04          	mov    %eax,0x4(%esp)
  8010c8:	8b 04 24             	mov    (%esp),%eax
  8010cb:	09 54 24 04          	or     %edx,0x4(%esp)
  8010cf:	89 fa                	mov    %edi,%edx
  8010d1:	d3 e0                	shl    %cl,%eax
  8010d3:	89 f1                	mov    %esi,%ecx
  8010d5:	89 44 24 08          	mov    %eax,0x8(%esp)
  8010d9:	8b 44 24 10          	mov    0x10(%esp),%eax
  8010dd:	d3 ea                	shr    %cl,%edx
  8010df:	89 e9                	mov    %ebp,%ecx
  8010e1:	d3 e7                	shl    %cl,%edi
  8010e3:	89 f1                	mov    %esi,%ecx
  8010e5:	d3 e8                	shr    %cl,%eax
  8010e7:	89 e9                	mov    %ebp,%ecx
  8010e9:	09 f8                	or     %edi,%eax
  8010eb:	8b 7c 24 10          	mov    0x10(%esp),%edi
  8010ef:	f7 74 24 04          	divl   0x4(%esp)
  8010f3:	d3 e7                	shl    %cl,%edi
  8010f5:	89 7c 24 0c          	mov    %edi,0xc(%esp)
  8010f9:	89 d7                	mov    %edx,%edi
  8010fb:	f7 64 24 08          	mull   0x8(%esp)
  8010ff:	39 d7                	cmp    %edx,%edi
  801101:	89 c1                	mov    %eax,%ecx
  801103:	89 14 24             	mov    %edx,(%esp)
  801106:	72 2c                	jb     801134 <__umoddi3+0x134>
  801108:	39 44 24 0c          	cmp    %eax,0xc(%esp)
  80110c:	72 22                	jb     801130 <__umoddi3+0x130>
  80110e:	8b 44 24 0c          	mov    0xc(%esp),%eax
  801112:	29 c8                	sub    %ecx,%eax
  801114:	19 d7                	sbb    %edx,%edi
  801116:	89 e9                	mov    %ebp,%ecx
  801118:	89 fa                	mov    %edi,%edx
  80111a:	d3 e8                	shr    %cl,%eax
  80111c:	89 f1                	mov    %esi,%ecx
  80111e:	d3 e2                	shl    %cl,%edx
  801120:	89 e9                	mov    %ebp,%ecx
  801122:	d3 ef                	shr    %cl,%edi
  801124:	09 d0                	or     %edx,%eax
  801126:	89 fa                	mov    %edi,%edx
  801128:	83 c4 14             	add    $0x14,%esp
  80112b:	5e                   	pop    %esi
  80112c:	5f                   	pop    %edi
  80112d:	5d                   	pop    %ebp
  80112e:	c3                   	ret    
  80112f:	90                   	nop
  801130:	39 d7                	cmp    %edx,%edi
  801132:	75 da                	jne    80110e <__umoddi3+0x10e>
  801134:	8b 14 24             	mov    (%esp),%edx
  801137:	89 c1                	mov    %eax,%ecx
  801139:	2b 4c 24 08          	sub    0x8(%esp),%ecx
  80113d:	1b 54 24 04          	sbb    0x4(%esp),%edx
  801141:	eb cb                	jmp    80110e <__umoddi3+0x10e>
  801143:	90                   	nop
  801144:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  801148:	3b 44 24 0c          	cmp    0xc(%esp),%eax
  80114c:	0f 82 0f ff ff ff    	jb     801061 <__umoddi3+0x61>
  801152:	e9 1a ff ff ff       	jmp    801071 <__umoddi3+0x71>
